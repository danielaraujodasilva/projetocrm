...
ALTER TABLE `studio_settings`
  ...
  ADD COLUMN IF NOT EXISTS `ai_pricing_page_enabled` TINYINT(1) NOT NULL DEFAULT 0 AFTER `business_rules`,
  ADD COLUMN IF NOT EXISTS `ai_pricing_page_url` VARCHAR(500) NULL AFTER `ai_pricing_page_enabled`,
  ADD COLUMN IF NOT EXISTS `ai_pricing_page_summary` MEDIUMTEXT NULL AFTER `ai_pricing_page_url`,
  ADD COLUMN IF NOT EXISTS `ai_pricing_page_raw_hash` VARCHAR(64) NULL AFTER `ai_pricing_page_summary`,
  ...
  ADD COLUMN IF NOT EXISTS `nvidia_document_model` VARCHAR(120) NOT NULL DEFAULT 'nvidia/nemoretriever-parse' AFTER `nvidia_document_api_key`,
  ADD COLUMN IF NOT EXISTS `nvidia_document_base_url` VARCHAR(180) NOT NULL DEFAULT 'https://integrate.api.nvidia.com/v1' AFTER `nvidia_document_model`,
  ADD COLUMN IF NOT EXISTS `nvidia_document_enabled` TINYINT(1) NOT NULL DEFAULT 1 AFTER `nvidia_document_base_url`,
  ADD COLUMN IF NOT EXISTS `nvidia_video_enabled` TINYINT(1) NOT NULL DEFAULT 1 AFTER `nvidia_document_enabled`,
  ADD COLUMN IF NOT EXISTS `nvidia_video_model` VARCHAR(120) NOT NULL DEFAULT 'meta/llama-3.2-90b-vision-instruct' AFTER `nvidia_video_enabled`,
  ADD COLUMN IF NOT EXISTS `nvidia_video_frame_count` TINYINT UNSIGNED NOT NULL DEFAULT 3 AFTER `nvidia_video_model`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_enabled` TINYINT(1) NOT NULL DEFAULT 0 AFTER `nvidia_video_frame_count`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_when_audio_only` TINYINT(1) NOT NULL DEFAULT 1 AFTER `ai_voice_reply_enabled`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_engine` VARCHAR(40) NOT NULL DEFAULT 'sapi' AFTER `ai_voice_reply_when_audio_only`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_xtts_sample_path` VARCHAR(500) NULL AFTER `ai_voice_reply_engine`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_xtts_language` VARCHAR(12) NOT NULL DEFAULT 'pt' AFTER `ai_voice_reply_xtts_sample_path`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_voice` VARCHAR(120) NULL AFTER `ai_voice_reply_xtts_language`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_rate` TINYINT NOT NULL DEFAULT 0 AFTER `ai_voice_reply_voice`,
  ADD COLUMN IF NOT EXISTS `ai_voice_reply_volume` TINYINT UNSIGNED NOT NULL DEFAULT 100 AFTER `ai_voice_reply_rate`,
  ADD COLUMN IF NOT EXISTS `ai_whatsapp_prompt` TEXT NULL AFTER `ai_voice_reply_volume`,
  ADD COLUMN IF NOT EXISTS `ai_provider` VARCHAR(20) NOT NULL DEFAULT 'nvidia' AFTER `ai_whatsapp_prompt`,
  ...
  ADD COLUMN IF NOT EXISTS `ai_chat_freestyle_mode` TINYINT(1) NOT NULL DEFAULT 0 AFTER `ai_api_base_url`,
