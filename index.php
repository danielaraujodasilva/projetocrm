...
if ($page === 'studio_home') {
    ...
    render_studio_shell('Inicio do CRM', 'Resumo operacional da instancia ' . (string)$user['studio_name'] . '.', 'home', function () use ($studio, $user) {
        ...
        echo '})();</script>';
        echo '<script>(function(){const modal=document.getElementById("homeDrilldownModal"),title=document.getElementById("homeDrilldownTitle"),summary=document.getElementById("homeDrilldownSummary"),body=document.getElementById("homeDrilldownBody");if(!modal||!title||!summary||!body)return;const esc=(v)=>String(v??"").replace(/[&<>"\x27]/g,(c)=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","\x27":"&#39;"}[c]||c));const card=(href,t,m,d)=>`<a class="drilldown-card"${href?` href="${esc(href)}"`:""}>${t?`<strong>${esc(t)}</strong>`:""}${m?`<div class="meta-line">${m}</div>`:""}${d?`<div class="muted">${esc(d)}</div>`:""}</a>`;function show(data){title.textContent=data.title||"Detalhe rapido";summary.textContent=data.summary||"";if(data.type==="availability"){const ranges=Object.entries(data.ranges||{});const defaultRange=data.default_range||"7d";const render=(rangeKey)=>{const pair=ranges.find(([k])=>k===rangeKey)||ranges.find(([k])=>k===defaultRange)||ranges[0]||["7d",{items:[],label:"7 dias"}];const rangeKeyReal=pair[0];const range=pair[1]||{items:[]};const options=ranges.map(([k,v])=>`<option value="${esc(k)}"${k===rangeKeyReal?" selected":""}>${esc(v.label||k)}</option>`).join("");const days=(range.items||[]).map(item=>`<details class="drilldown-card drilldown-details" open><summary><strong>${esc(item.label)}</strong><span class="muted">${esc(item.allowed?(item.free+" vagas livres"):"Fora dos dias permitidos")}</span></summary><div class="drilldown-detail-list"><div><span class="muted">Horarios livres</span>${(item.free_slots||[]).length?`<div class="drilldown-chip-row">${item.free_slots.map(slot=>`<button type="button" class="drilldown-chip" data-availability-date="${esc(item.date)}">${esc(slot)}</button>`).join("")}</div>`:`<p class="muted">Sem horarios livres nesse dia.</p>`}</div>${(item.booked||[]).length?`<div style="margin-top:10px"><span class="muted">Ocupados</span><div class="drilldown-chip-row">${item.booked.map(appt=>`<button type="button" class="drilldown-chip secondary" data-appointment-id="${esc(appt.id||0)}" data-appointment-date="${esc(item.date)}">${esc(appt.time)}${appt.customer_name?` • ${esc(appt.customer_name)}`:""}</button>`).join("")}</div></div>`:""}</div></details>`).join("");body.innerHTML=`<div class="availability-toolbar"><label class="field" style="max-width:240px"><span class="muted">Período</span><select id="availabilityRangeSelect">${options}</select></label><p class="muted">Mostrando ${esc(range.label||rangeKeyReal)}. Toque num dia ou horario para navegar direto.</p></div><div class="drilldown-grid availability-grid">${days||`<div class="drilldown-card"><strong>Nenhuma vaga livre encontrada</strong><div class="muted">Nesse período não apareceu nenhum horário livre rápido dentro das regras do estúdio.</div></div>`}</div>`;setTimeout(()=>{const sel=document.getElementById("availabilityRangeSelect");if(sel){sel.addEventListener("change",function(){render(this.value);});}},0);};render(defaultRange);}else if(data.type==="finance"){body.innerHTML=`<div class="drilldown-grid">${(data.items||[]).map(item=>card("",item.value,item.label,item.detail||"")).join("")}</div>`;}else if(data.type==="appointments"){body.innerHTML=`<div class="drilldown-grid">${(data.items||[]).map(item=>card(item.id?`index.php?page=studio_agenda&date=${encodeURIComponent(item.appointment_date||"")}&appointment_id=${encodeURIComponent(item.id)}#appointment-form`:"",item.customer_name||item.display_name||item.title||"Agendamento",`<span>${esc(item.appointment_date||item.last_message_at||"-")}</span><span>${esc(item.start_time?item.start_time.slice(0,5):"")}</span>`,item.description||item.last_message_preview||item.title||"-")).join("")}</div>`;}else if(data.type==="whatsapp"){body.innerHTML=`<div class="drilldown-grid">${(data.items||[]).map(item=>card(`index.php?page=studio_whatsapp_conversation&id=${encodeURIComponent(item.id)}`,item.display_name||item.phone||"Contato",`<span>${esc(item.last_message_at||"-")}</span><span>${esc(item.attendance_mode||"-")}</span>`,item.last_message_preview||"-")).join("")}</div>`;}else if(data.type==="table"){body.innerHTML=`<div class="drilldown-grid">${(data.items||[]).map(item=>card("",item.name||item.title||item.customer_name||item.display_name||item.phone||"Item",`${item.status?`<span>${esc(item.status)}</span>`:""}${item.phone?`<span>${esc(item.phone)}</span>`:""}${item.last_message_at?`<span>${esc(item.last_message_at)}</span>`:""}${item.value?`<span>${esc(item.value)}</span>`:""}`,item.description||item.last_message_preview||item.interest||"")).join("")}</div>`;}else{body.innerHTML=`<div class="drilldown-card">${esc(data.summary||"")}</div>`;}modal.classList.remove("hidden");return false;}window.openHomeDrilldown=function(key){try{return show((window.homeDrilldowns||{})[key]||null);}catch(e){console.error(e);return false;}};document.querySelectorAll("[data-home-focus]").forEach(function(btn){btn.addEventListener("click",function(){return window.openHomeDrilldown(btn.getAttribute("data-home-focus")||"");});});})();</script>';
