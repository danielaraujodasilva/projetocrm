...
if ($page === 'studio_whatsapp_workspace') {
    ...
    render_studio_shell('Workspace WhatsApp', 'Leitura tipo WhatsApp Web com ferramentas do CRM.', 'whatsapp', function () use ($studio) {
        ...
        if (!$conversation) {
            ...
            echo '<button class="wa-web-icon-btn" type="button" id="chatEmojiButton" aria-label="Emoji"><i class="fa-regular fa-face-smile"></i></button>';
        ...
        echo 'if(toolsButton&&toolsOverlay&&toolsOverlayBody&&toolsRail){ toolsButton.addEventListener("click",()=>{ if(window.innerWidth<=1180){ toolsOverlayBody.innerHTML=toolsRail.innerHTML; toolsOverlay.classList.remove("hidden"); return; } document.getElementById("workspaceToolsDesktop")?.scrollIntoView({ behavior:"smooth", block:"start" }); document.getElementById("workspaceToolsDesktop")?.classList.add("wa-web-tools-highlight"); setTimeout(()=>document.getElementById("workspaceToolsDesktop")?.classList.remove("wa-web-tools-highlight"), 1200); }); } closeToolsOverlay?.addEventListener("click",()=>toolsOverlay.classList.add("hidden")); toolsOverlay?.addEventListener("click",(event)=>{ if(event.target===toolsOverlay) toolsOverlay.classList.add("hidden"); });';
