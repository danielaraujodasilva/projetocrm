...
if ($page === 'lead_public_update') {
    ...
    render_public_page('Atualizar cadastro', 'Complete seus dados para agilizar o atendimento.', function () use ($lead, $leadId, $token, $customerSeed) {
        ...
        echo '<div class="public-progress"><div><strong style="display:block;color:#0f172a">Etapa</strong><span class="muted" id="wizard-label">1 de 4 Â· Dados bÃ¡sicos</span></div><div class="track"><span id="wizard-bar"></span></div></div>';
        ...
        echo '<script>
            (function(){
                ...
                const show = (n) => {
                    ...
                    if (label) label.textContent = `${step} de 4 Â· ${stepNames[step - 1] || ""}`;
                ...
                form.addEventListener("submit", async (ev) => {
                    if (step < 4) {
                        ...
                        if (!consent1?.checked || !consent2?.checked || !consent3?.checked) {
                            ev.preventDefault();
                            alert("Marque os consentimentos obrigatÃ³rios: cadastro, saÃºde e veracidade.");
                        }
                    }
                });
                autosave("opened");
...
if ($page === 'studio_customer') {
    ...
    render_studio_shell('Ficha do cliente', 'Historico completo de lead, WhatsApp e agenda.', 'customers', function () use ($studio) {
        ...
        if ($publicUpdateUrl !== '') {
            ...
            echo '<div class="actions" style="margin-top:14px;gap:8px;flex-wrap:wrap"><a class="btn secondary" href="' . h($publicUpdateUrl) . '" target="_blank" rel="noopener">Abrir cadastro pÃºblico</a><button type="button" class="btn secondary" data-copy-link="' . h($publicUpdateUrl) . '">Copiar link</button><a class="btn secondary" href="https://wa.me/?text=' . h(rawurlencode($publicShareMessage)) . '" target="_blank" rel="noopener">Enviar no WhatsApp</a></div>';
        } else {
            echo '<p class="muted" style="margin-top:14px">Este cliente ainda nÃ£o tem um lead vinculado para gerar o link pÃºblico de atualizaÃ§Ã£o.</p>';
        }
...
if ($page === 'studio_leads') {
    ...
    render_studio_shell('Funil de Leads', 'Acompanhe oportunidades, orÃ§amentos e agendamentos do estÃºdio.', 'leads', function () use ($studio) {
        ...
        echo '<div class="actions" style="justify-content:space-between;align-items:flex-start"><div><h2>Funil de Leads</h2><p class="muted">Etapa s ordenadas, total por coluna e cartÃµes com aÃ§Ã£o comercial.</p></div><span class="badge">' . h((string)count($openLeads)) . ' leads abertos</span></div>';
        if (!$allLeads) {
...
if ($page === 'studio_agenda') {
    ...
    render_studio_shell('Agenda', 'Calendario, tatuadores e proximos atendimentos.', 'agenda', function () use ($studio) {
        ...
        if ($selectedAppointment) {
            ...
            if ($healthAlerts) {
                echo '<div class="panel" style="margin-top:12px;border-left:4px solid #eab308;background:#fffbeb"><strong>Alertas de saÃºde</strong><div class="stack-list" style="margin-top:10px">';
        ...
        echo '<section class="grid cols-2" style="margin-top:16px">';
        echo '<button type="button" class="panel dashboard-stat" id="openNewSlotOverlay"><p class="metric">Novo horÃ¡rio</p><p class="muted">Abrir formulÃ¡rio em overlay</p></button>';
        ...
        echo '</section>';
        echo '</div>';
        echo '<div id="agendaTableSource" hidden><section class="panel" style="margin-top:16px"><div class="actions" style="justify-content:space-between;align-items:flex-start"><div><h2>Agenda cadastrada</h2><p class="muted">PÃ¡gina ' . h((string)$appointmentsPage) . ' de ' . h((string)$appointmentsTotalPages) . '</p></div>';
...
if ($page === 'studio_whatsapp') {
    ...
    render_studio_shell('WhatsApp', 'Central de conversas e sessao Baileys deste estudio.', 'whatsapp', function () use ($studio) {
        ...
        echo '<button type="button" class="panel quick-action-card" id="openWhatsAppStatusOverlay"><strong>' . h($serviceStateLabel) . '</strong><span>Status do WhatsApp</span><small>Ver conexÃ£o, pareamento e aÃ§Ãµes</small></button>';
        ...
        echo '<button type="button" class="panel quick-action-card" id="openWhatsAppReadingOverlay"><strong>' . h($summary['analyzed']) . '</strong><span>Leitura rÃ¡pida</span><small>Resumo do fluxo atual</small></button>';
        ...
        echo '<form method="post" class="inline-form">' . csrf_field() . '<input type="hidden" name="action" value="reset_whatsapp_session"><button class="btn secondary" type="submit">Limpar sessÃ£o</button></form>';
        echo '<form method="post" class="inline-form">' . csrf_field() . '<input type="hidden" name="action" value="restart_whatsapp_service"><button class="btn secondary" type="submit">Reiniciar serviÃ§o</button></form>';
        echo '</div>';
        ...
        if ($conversationsTotalPages > 1) {
            ...
            if ($conversationsPage < $conversationsTotalPages) {
                echo '<a class="btn secondary" href="' . h(app_url('studio_whatsapp', ['filter' => $filters['filter'] ?: 'all', 'q' => $filters['q'] !== '' ? $filters['q'] : null, 'mode' => $filters['mode'] !== '' ? $filters['mode'] : null, 'needs_human' => $filters['needs_human'] ? 1 : null, 'min_score' => $filters['min_score'] > 0 ? $filters['min_score'] : null, 'wa_page' => $conversationsPage + 1])) . '">PrÃ³xima</a>';
            }
            echo '</div>';
        }
        echo '</section></div>';
...
if ($page === 'studio_finance') {
    ...
    render_studio_shell('Financeiro', 'Despesas e leitura simples do resultado mensal.', 'finance', function () use ($studio) {
        ...
        echo '</section>';
        echo '<div id="financeAgendaSource" hidden><div class="panel" style="margin:0"><div class="actions" style="justify-content:space-between"><h2>Agenda no mÃªs</h2><a class="btn secondary" href="' . h(app_url('studio_agenda')) . '">Abrir agenda</a></div><p class="muted">Resumo financeiro ligado aos agendamentos do perÃ­odo.</p></div></div>';
        ...
        echo '<div class="panel" style="margin-top:16px"><div class="actions" style="justify-content:space-between"><h2>Despesas recentes</h2><span class="muted">Lista completa do mÃªs</span></div>';
...
'detail' => format_date_pt((string)$appointment['appointment_date']) . ' Ã s ' . substr((string)$appointment['start_time'], 0, 5) . ' Â· ' . format_money(appointment_display_amount($appointment['value'] ?? 0)),
        ...
        if ($todayHealthAlerts) {
            $alerts[] = [
                'title' => 'Alertas de saÃºde de hoje',
                ...
                'items' => array_map(static function (array $appointment): array {
                    ...
                    return [
                        'label' => ($appointment['customer_name'] ?: 'Atendimento'),
                        'detail' => format_date_pt((string)$appointment['appointment_date']) . ' Â· ' . substr((string)$appointment['start_time'], 0, 5) . (empty($labels) ? '' : ' Â· ' . implode(' Â· ', array_slice($labels, 0, 3))),
                        'href' => $href,
                    ];
                }, $todayHealthAlerts),
            ];
...
if ($page === 'studio_settings') {
    ...
    render_studio_shell('Configuracoes do estudio', 'Regras comerciais e preparacao dos modulos de IA/WhatsApp.', 'settings', function () use ($studio) {
        ...
        echo '<div class="settings-overview-grid">';
        $settingsCards = [
            'studio' => ['EstÃºdio', 'Dados base e integraÃ§Ã£o'],
            'agenda' => ['Agenda', 'Regras de horÃ¡rio e duraÃ§Ã£o'],
            'whatsapp' => ['WhatsApp', 'Entrada e comportamento'],
            'ia' => ['IA', 'Modelo, chave e automaÃ§Ã£o'],
            'quick_replies' => ['Respostas rÃ¡pidas', 'Biblioteca do atendimento'],
        ...
        echo '<div class="grid cols-2">';
        echo '<div class="field"><label>PadrÃ£o das novas conversas WhatsApp</label><select name="whatsapp_default_mode">';
        render_options(['human' => 'Humano atende primeiro', 'bot' => 'IA atende primeiro'], (string)($settings['whatsapp_default_mode'] ?? 'human'));
        echo '</select></div>';
        echo '<div class="field"><label>URL do serviÃ§o Baileys</label><input name="whatsapp_service_url" value="' . h($settings['whatsapp_service_url'] ?? 'http://localhost:3010') . '"></div>';
        echo '</div>';
        ...
        echo '<div class="field"><label>TÃ­tulo</label><input name="title" required></div>';
...
function render_calendar_block(array $appointment): void
{
    ...
    if (studio_appointment_health_alerts_from_row($appointment)) {
        echo '<span class="badge warn">saÃºde</span>';
    }
    echo '</a>';
}
...
echo '<td>' . h(format_money($appointmentValue)) . '<br><span class="muted">Sinal ' . h(format_money($appointmentDeposit)) . '</span></td>';
        echo '<td><span class="badge">' . h($appointment['status']) . '</span>' . (studio_appointment_health_alerts_from_row($appointment) ? '<br><span class="badge warn">saÃºde</span>' : '') . '<br><a class="btn tiny secondary" href="' . h($href) . '">Abrir</a></td>';
        echo '</tr>';
    }
    echo '</tbody></table>';
}
