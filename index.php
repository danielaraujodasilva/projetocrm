...
if ($page === 'studio_whatsapp_conversation') {
    ...
    render_studio_shell('Conversa WhatsApp', 'Historico, atendimento e envio direto.', 'whatsapp', function () use ($studio) {
        ...
        echo 'document.querySelectorAll("[data-mode-toggle]").forEach(button => button.addEventListener("click", () => { const action = button.dataset.modeToggle === "bot" ? "bot" : "assumir"; fetch(window.location.pathname + window.location.search, { method: "POST", headers: { "X-Requested-With": "XMLHttpRequest" }, body: new URLSearchParams({ action: action === "bot" ? "update_whatsapp_profile" : "update_whatsapp_profile", conversation_id: String(conversationId), attendance_mode: action === "bot" ? "bot" : "human" }) }).then(async response => { if (!response.ok) throw new Error("Nao foi possivel atualizar o atendimento."); location.reload(); }).catch(error => alert(error.message)); }));';
        echo 'document.querySelectorAll("[data-status-set]").forEach(button => button.addEventListener("click", () => { fetch(window.location.pathname + window.location.search, { method: "POST", headers: { "X-Requested-With": "XMLHttpRequest" }, body: new URLSearchParams({ action: "update_whatsapp_profile", conversation_id: String(conversationId), status: button.dataset.statusSet || "novo" }) }).then(async response => { if (!response.ok) throw new Error("Nao foi possivel atualizar o status."); location.reload(); }).catch(error => alert(error.message)); }));';
        ...
        echo 'const pollConversation = async () => { try { const response = await fetch(`api_chat.php?id=${encodeURIComponent(conversationId)}`, { cache: "no-store" }); const data = await response.json().catch(() => null); if (!data?.ok) return; const count = Array.isArray(data.mensagens) ? data.mensagens.length : 0; if (count !== lastMessageCount) { lastMessageCount = count; location.reload(); } } catch (error) {} };';
