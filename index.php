...
function render_public_agent_page(): void
{
    ...
    echo '<li>Ler informações pÃºblicas bÃ¡sicas sem esbarrar na tela de login.</li>';
...
if ($page === 'studio_customer') {
    ...
    render_studio_shell('Ficha do cliente', 'Historico completo de lead, WhatsApp e agenda.', 'customers', function () use ($studio) {
        ...
        echo '<div class="grid cols-2"><div class="field"><label>CondiÃ§Ãµes de saúde</label><textarea name="health_conditions">' . h($customer['health_conditions'] ?? '') . '</textarea></div><div class="field"><label>CondiÃ§Ãµes de pele</label><textarea name="skin_conditions">' . h($customer['skin_conditions'] ?? '') . '</textarea></div></div>';
...
if ($page === 'studio_agenda') {
    ...
    render_studio_shell('Agenda', 'Calendario, tatuadores e proximos atendimentos.', 'agenda', function () use ($studio) {
        ...
        if (!$nextAvailableSlots) {
            echo '<p class="muted">Não foi possÃ­vel calcular horÃ¡rios livres neste recorte.</p>';
        } else {
...
if ($page === 'studio_settings') {
    ...
    render_studio_shell('Configuracoes do estudio', 'Regras comerciais e preparacao dos modulos de IA/WhatsApp.', 'settings', function () use ($studio) {
        ...
        echo '<div class="field"><label>Regras e informações para IA</label><textarea name="business_rules" placeholder="Exemplo: EstÃºdio aberto de terÃ§a a sÃ¡bado. Dois tatuadores. Responder sempre em portuguÃªs do Brasil. Quando o cliente pedir agendamento, considerar sinal obrigatÃ³rio. Não inventar preÃ§o. Se faltar informaÃ§Ã£o, perguntar sÃ³ uma coisa por vez. Priorize datas, horÃ¡rios e referÃªncias reais do estÃºdio.">' . h($settings['business_rules'] ?? $studio['business_rules'] ?? '') . '</textarea><small class="muted">Esse texto entra no contexto da IA. Aqui vale escrever regras reais do estÃºdio, tom de atendimento, limites e informações que a IA deve respeitar sempre.</small></div>';
