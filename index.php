...
if ($page === 'studio_whatsapp_mobile') {
    ...
    if (!$conversation) {
        ...
        echo '<button type="button" class="mobile-wa-action" id="mobileOpenSchedule" onclick="window.mobileOpenSchedulePanel && window.mobileOpenSchedulePanel(event)"><i class="fa-solid fa-calendar-days"></i><span>Agendar</span></button>';
        ...
        echo '<button class="mobile-wa-icon-btn" type="button" id="mobileOpenEmojiPanel" aria-label="Emoji" title="Emoji" onclick="window.mobileOpenEmojiPanel && window.mobileOpenEmojiPanel(event)"><i class="fa-regular fa-face-smile"></i><span>Emoji</span></button>';
        echo '<button class="mobile-wa-icon-btn" type="button" id="mobileOpenAttachmentPicker" aria-label="Anexar" title="Anexar" onclick="window.mobileOpenAttachmentPicker && window.mobileOpenAttachmentPicker(event)"><i class="fa-solid fa-paperclip"></i><span>Anexar</span></button>';
        echo '<button class="mobile-wa-icon-btn" type="button" id="mobileRecordAudioButton" aria-label="Audio" title="Áudio" onclick="window.mobileToggleRecording && window.mobileToggleRecording(event)"><i class="fa-solid fa-microphone"></i><span>Áudio</span></button>';
    ...
    echo '<button type="button" class="nav-chip' . ($mobileView === 'actions' ? ' active' : '') . '" id="mobileBottomActionsButton" onclick="window.mobileOpenGlobalActions && window.mobileOpenGlobalActions(event)"><i class="fa-solid fa-bolt"></i><span>Ações</span></button>';
    echo '</div>';
    ...
    echo '<button type="button" class="mobile-wa-action" id="mobileGlobalRefreshAction" onclick="window.mobileReloadMobile && window.mobileReloadMobile(event)"><span aria-hidden="true">↻</span><span>Atualizar lista</span></button>';
    if ($conversationId > 0) {
    ...
    echo '</div>';
    echo '</div>';
    echo '<script>(function(){const messages=document.getElementById("mobileWaMessages"); if(messages){ const scrollNow=()=>{ messages.scrollTop=messages.scrollHeight; if(messages.lastElementChild && messages.lastElementChild.scrollIntoView){ messages.lastElementChild.scrollIntoView({block:"end",inline:"nearest"}); } }; scrollNow(); setTimeout(scrollNow,120); setTimeout(scrollNow,360); setTimeout(scrollNow,900); window.addEventListener("load", scrollNow, { once:true }); }})();</script>';
