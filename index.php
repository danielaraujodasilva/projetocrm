...
if ($page === 'lead_public_update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$dbStatus['ok']) {
        ...
        exit('Banco do estÃºdio indisponÃ­vel.');
    }
    ...
    if ($payload['data_processing_consent'] !== '1' || $payload['health_data_consent'] !== '1' || $payload['truthfulness_confirmed'] !== '1') {
        flash_set('error', 'Marque os consentimentos obrigatÃ³rios para enviar.');
...
if ($page === 'lead_public_update') {
    ...
    if (!empty($_GET['done'])) {
        ...
        render_public_page('Cadastro enviado', 'Obrigado por completar seu cadastro.', function () use ($waUrl) {
            echo '<div class="public-page-wrap" style="padding:40px 16px"><section class="panel" style="max-width:720px;margin:0 auto;text-align:center"><h1>Cadastro enviado!</h1><p class="muted">Recebi suas informaÃ§Ãµes. Agora posso seguir seu atendimento com mais seguranÃ§a.</p>';
            if ($waUrl !== '') {
    ...
    render_public_page('Atualizar cadastro', 'Complete seus dados para agilizar o atendimento.', function () use ($lead, $leadId, $token, $customerSeed) {
        ...
        echo '<style>
            ...
            .public-lead-section{border:1px solid rgba(15,23,42,.08);border-radius:20px;background:#fff;padding:18px;box-shadow:0 10px 30px rgba(15,23,42,.05)}
        ...
        echo '<span class="public-lead-kicker">Ficha rÃ¡pida</span>';
        echo '<h1>Complete seu cadastro</h1>';
        echo '<p>Leva poucos minutos e ajuda a fazer seu atendimento com mais seguranÃ§a.</p>';
        echo '<div class="public-lead-btn-row" style="margin-top:18px"><a class="btn" href="#lead-form">ComeÃ§ar cadastro</a></div>';
        echo '<div class="muted" style="margin-top:10px;color:#dbeafe">Seu progresso Ã© salvo automaticamente.</div>';
        echo '</section>';
        ...
        echo '<div class="public-lead-chip" style="padding:12px 10px"><strong>3</strong><span>SaÃºde</span></div>';
        ...
        echo '<section class="public-lead-section public-step is-active" data-step="1"><div class="actions" style="justify-content:space-between;align-items:flex-start"><h2 style="margin-top:0">Dados bÃ¡sicos</h2><span class="badge">1 de 4</span></div><p class="public-lead-note">Essas informaÃ§Ãµes identificam seu cadastro.</p><div class="grid cols-2"><div class="field"><label>Nome completo</label><input name="name" value="' . h($value('name', (string)($lead['name'] ?? ''))) . '" placeholder="Seu nome completo"></div><div class="field"><label>Data de nascimento</label><input type="date" name="birth_date" value="' . h($value('birth_date')) . '"></div></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>CPF / documento</label><input name="document_number" value="' . h($value('document_number')) . '" placeholder="CPF ou outro documento"></div><div class="field"><label>ProfissÃ£o</label><input name="occupation" value="' . h($value('occupation')) . '" placeholder="Opcional"></div></div><div class="field" style="margin-top:12px"><label>Instagram</label><input name="instagram" value="' . h($value('instagram')) . '" placeholder="@seuusuario"></div></section>';
        echo '<section class="public-lead-section public-step" data-step="2" hidden><div class="actions" style="justify-content:space-between;align-items:flex-start"><h2 style="margin-top:0">Contato</h2><span class="badge">2 de 4</span></div><p class="public-lead-note">Usaremos esses dados para atendimento e organizaÃ§Ã£o.</p><div class="grid cols-2"><div class="field"><label>WhatsApp</label><input name="phone" value="' . h($value('phone', (string)($lead['phone'] ?? ''))) . '" placeholder="(00) 00000-0000"></div><div class="field"><label>Email</label><input name="email" type="email" value="' . h($value('email')) . '" placeholder="Opcional"></div></div><div class="public-lead-section-toggle" style="margin-top:14px"><button type="button" class="btn secondary" id="toggle-address">Adicionar endereÃ§o</button></div><div id="address-fields" class="public-lead-advanced" style="display:none;margin-top:14px"><div class="grid cols-3"><div class="field"><label>CEP</label><input name="address_zip" value="' . h($value('address_zip')) . '" placeholder="00000-000"></div><div class="field"><label>Cidade</label><input name="address_city" value="' . h($value('address_city')) . '"></div><div class="field"><label>Estado</label><input name="address_state" value="' . h($value('address_state')) . '"></div></div><div class="grid cols-3" style="margin-top:12px"><div class="field"><label>Bairro</label><input name="address_neighborhood" value="' . h($value('address_neighborhood')) . '"></div><div class="field"><label>Rua</label><input name="address_street" value="' . h($value('address_street')) . '"></div><div class="field"><label>NÃºmero</label><input name="address_number" value="' . h($value('address_number')) . '"></div></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Complemento</label><input name="address_complement" value="' . h($value('address_complement')) . '"></div><div class="field"><label>ReferÃªncia</label><input name="address_reference" value="' . h($value('address_reference')) . '"></div></div></div></section>';
        echo '<section class="public-lead-section public-step" data-step="3" hidden><div class="actions" style="justify-content:space-between;align-items:flex-start"><h2 style="margin-top:0">Anamnese rÃ¡pida</h2><span class="badge">3 de 4</span></div><p class="public-lead-note">Responda o que souber. Isso ajuda na seguranÃ§a do procedimento.</p><div class="grid cols-2"><div class="field"><label>Tem alergia?</label><div class="public-choicebar" data-group="allergies"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="allergies" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="allergies_details" rows="3"></textarea></div></div><div class="field"><label>Usa medicamento contÃ­nuo ou importante?</label><div class="public-choicebar" data-group="medications"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="medications" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="medications_details" rows="3"></textarea></div></div></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Usa anticoagulante?</label><div class="public-choicebar" data-group="anticoagulants"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="anticoagulants" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="anticoagulants_details" rows="3"></textarea></div></div><div class="field"><label>Tem diabetes?</label><div class="public-choicebar" data-group="diabetes"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="diabetes" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="diabetes_details" rows="3"></textarea></div></div></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Tem pressÃ£o alta?</label><div class="public-choicebar" data-group="health_conditions"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="health_conditions" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="health_conditions_details" rows="3"></textarea></div></div><div class="field"><label>Tem alguma condiÃ§Ã£o de pele importante?</label><div class="public-choicebar" data-group="skin_conditions"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="skin_conditions" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="skin_conditions_details" rows="3"></textarea></div></div></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Tem histÃ³rico de queloide?</label><div class="public-choicebar" data-group="keloid_history"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="keloid_history" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="keloid_history_details" rows="3"></textarea></div></div><div class="field"><label>Tem problemas de cicatrizaÃ§Ã£o?</label><div class="public-choicebar" data-group="healing_issues"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="healing_issues" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="healing_issues_details" rows="3"></textarea></div></div></div><div class="grid cols-1" style="margin-top:12px"><div class="field"><label>EstÃ¡ grÃ¡vida ou amamentando?</label><div class="public-choicebar" data-group="pregnant_or_breastfeeding"><button type="button" class="public-choice" data-value="NÃ£o">NÃ£o</button><button type="button" class="public-choice" data-value="Sim">Sim</button><button type="button" class="public-choice" data-value="NÃ£o sei">NÃ£o sei</button></div><input type="hidden" name="pregnant_or_breastfeeding" value=""><div class="field public-step-extra" style="display:none;margin-top:10px"><label>Descreva rapidamente.</label><textarea name="pregnant_or_breastfeeding_details" rows="3"></textarea></div></div></div></section>';
        echo '<section class="public-lead-section public-step" data-step="4" hidden><div class="actions" style="justify-content:space-between;align-items:flex-start"><h2 style="margin-top:0">Consentimentos</h2><span class="badge">4 de 4</span></div><p class="public-lead-note">Marque para confirmar e enviar.</p><div class="public-lead-consent-grid"><label class="checkline"><input type="checkbox" name="data_processing_consent" value="1"> Autorizo o uso dos meus dados para cadastro, atendimento e organização do procedimento</label><label class="checkline"><input type="checkbox" name="health_data_consent" value="1"> Autorizo o uso das informações de saúde para avaliação de segurança do procedimento</label><label class="checkline"><input type="checkbox" name="truthfulness_confirmed" value="1"> Declaro que as informações preenchidas são verdadeiras</label><label class="checkline"><input type="checkbox" name="marketing_opt_in" value="1"> Aceito receber promoções pelo WhatsApp</label><label class="checkline"><input type="checkbox" name="email_opt_in" value="1"> Aceito receber comunicações por email</label><label class="checkline"><input type="checkbox" name="share_before_after_opt_in" value="1"> Autorizo uso de fotos e vídeos em portfólio</label><label class="checkline"><input type="checkbox" name="social_network_opt_in" value="1"> Autorizo marcação em redes sociais</label><label class="checkline"><input type="checkbox" name="whatsapp_opt_in" value="1"> Autorizo contato por WhatsApp</label></div><p class="muted" style="margin-top:14px">Seus dados serão usados apenas para cadastro, atendimento e segurança do procedimento.</p><div class="public-lead-section-toggle" style="margin-top:14px"><button type="button" class="btn secondary" id="toggle-address">Adicionar endereço</button><button class="btn" type="submit">Enviar e agilizar meu atendimento</button></div><div id="address-fields" class="public-lead-advanced" style="display:none;margin-top:14px"><div class="grid cols-3"><div class="field"><label>CEP</label><input name="address_zip" value="' . h($value('address_zip')) . '" placeholder="00000-000"></div><div class="field"><label>Cidade</label><input name="address_city" value="' . h($value('address_city')) . '"></div><div class="field"><label>Estado</label><input name="address_state" value="' . h($value('address_state')) . '"></div></div><div class="grid cols-3" style="margin-top:12px"><div class="field"><label>Bairro</label><input name="address_neighborhood" value="' . h($value('address_neighborhood')) . '"></div><div class="field"><label>Rua</label><input name="address_street" value="' . h($value('address_street')) . '"></div><div class="field"><label>Número</label><input name="address_number" value="' . h($value('address_number')) . '"></div></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Complemento</label><input name="address_complement" value="' . h($value('address_complement')) . '"></div><div class="field"><label>Referência</label><input name="address_reference" value="' . h($value('address_reference')) . '"></div></div></div></section>';
        ...
        echo '<script>
            (function(){
                ...
                const stepNames = ["Dados bÃ¡sicos","Contato","Anamnese","Consentimentos"];
                let step = 1;
                const show = (n) => {
                    ...
                    if (label) label.textContent = stepNames[step - 1] || "";
                ...
                if (next) next.addEventListener("click", async () => { const ev = step === 1 ? "step_contact_completed" : step === 2 ? "step_project_completed" : "step_health_completed"; await autosave(ev); show(step + 1); });
                const advancedToggle = document.getElementById("toggle-address");
                ...
                const groups = ["allergies","medications","diabetes","anticoagulants","keloid_history","healing_issues","skin_conditions","pregnant_or_breastfeeding"];
                groups.forEach((name) => {
                ...
                form.addEventListener("submit", async (ev) => {
                    if (step < 4) {
                        ...
                        if (!consent1?.checked || !consent2?.checked || !consent3?.checked) {
                            ev.preventDefault();
                            alert("Marque os consentimentos obrigatÃ³rios para enviar.");
                        }
                    }
                });
                autosave("opened");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $page !== 'public_plans' && $page !== 'lead_public_update') {
    ...
    try {
        ...
        if ($action === 'public_lead_update') {
            ...
            $dbStatus = studio_db_status_for($studio);
            if (!$dbStatus['ok']) {
                throw new RuntimeException('Banco do estÃºdio indisponÃ­vel.');
            }
            $lead = studio_find_lead($studio, $leadId);
            if (!$lead || trim((string)($lead['public_update_token'] ?? '')) !== $token) {
                throw new RuntimeException('Link expirado ou invalido.');
...
if ($page === 'studio_whatsapp_live') {
    ...
    if (!$studio) {
        ...
        echo json_encode([
            'ok' => false,
            'error' => 'Sessao do estudio expirou. FaÃ§a login novamente.',
...
function render_public_agent_page(): void
{
    ...
    echo '<!doctype html><html lang="pt-BR"><head><meta charset="utf-8">';
    echo '<meta name="viewport" content="width=device-width, initial-scale=1">';
    echo '<title>projetocrm Â· PÃ¡gina pÃºblica para agentes</title>';
    echo '<meta name="description" content="PÃ¡gina pÃºblica de verificaÃ§Ã£o do CRM do estÃºdio, acessÃ­vel sem login para agentes e validadores.">';
    echo '<style>
    ...
    echo '<section class="hero"><h1>projetocrm</h1><p>Esta Ã© uma pÃ¡gina pÃºblica de verificaÃ§Ã£o para agentes, navegadores automatizados e validadores. As Ã¡reas operacionais do CRM exigem autenticaÃ§Ã£o, mas esta rota foi criada justamente para ser acessÃ­vel sem login.</p>';
    echo '<div class="actions"><a class="btn primary" href="' . h(app_url('studio_login')) . '">Login do estÃºdio</a><a class="btn" href="' . h(app_url('login')) . '">Painel gerente</a></div></section>';
    echo '<div class="grid">';
    ...
    echo '<li>Confirmar que o domÃ­nio e a aplicaÃ§Ã£o estÃ£o online.</li>';
    echo '<li>Ler informaÃ§Ãµes pÃºblicas bÃ¡sicas sem esbarrar na tela de login.</li>';
    echo '<li>Descobrir as rotas corretas de autenticaÃ§Ã£o do CRM.</li>';
    echo '</ul></section>';
    echo '<section class="panel" style="margin-top:18px"><h2>Resumo tÃ©cnico</h2><pre>' . h(json_encode([
        'app' => 'projetocrm',
...
if ($page === 'public_plans') {
    ...
    render_public_page('Planos do CRM para EstÃºdios de Tatuagem', 'CRM para tatuadores com leads, agenda, WhatsApp, financeiro, relatÃ³rios e IA.', function () use ($publicPlans) {
        ...
        echo '<span class="public-kicker">Venda mais. Responda melhor. Organize sua operaÃ§Ã£o.</span>';
        echo '<h1>CRM para estÃºdios de tatuagem</h1>';
        echo '<p class="public-lead">Organize leads, clientes, agenda, WhatsApp e financeiro em um sÃ³ lugar.</p>';
        ...
        echo '<p class="muted">Pensado para vender para estÃºdios reais, com preÃ§o, limite e recurso editÃ¡veis pelo painel administrativo.</p>';
        ...
        if (!$publicPlans) {
            echo '<section class="public-section"><div class="public-empty">Nenhum plano disponÃ­vel no momento.</div></section>';
            return;
        }

        echo '<section class="public-section" id="planos">';
        echo '<div class="section-head"><h2>Escolha o plano certo para o seu estÃºdio</h2><p>Os valores, limites e recursos abaixo vÃªm do banco e acompanham as alteraÃ§Ãµes do painel administrativo.</p></div>';
        ...
        echo '</tr></thead><tbody>';
        $compareRows = [
            ['label' => 'UsuÃ¡rios', 'key' => 'user_limit'],
            ...
            ['label' => 'AutomaÃ§Ãµes', 'key' => 'allow_automations'],
            ['label' => 'RelatÃ³rios avanÃ§ados', 'key' => 'allow_advanced_reports'],
            ['label' => 'Multi-estÃºdio', 'key' => 'allow_multi_studio'],
        ];
        foreach ($compareRows as $row) {
            ...
            foreach ($publicPlans as $plan) {
                ...
                if (str_starts_with($key, 'allow_')) {
                    echo '<td>' . (!empty($value) ? '<span class="badge ok">Sim</span>' : '<span class="badge warn">NÃ£o</span>') . '</td>';
                } else {
        ...
        echo '<div class="section-head"><h2>Para quem Ã©</h2><p>Os textos abaixo podem ser refinados depois pelo painel, mas jÃ¡ ajudam o visitante a se enxergar em cada plano.</p></div>';
        ...
        echo '<div class="section-head"><h2>BenefÃ­cios do CRM</h2><p>Um resumo simples do que o sistema resolve no dia a dia do estÃºdio.</p></div>';
        echo '<div class="public-benefits-grid">';
        foreach ([
            'Pare de perder orÃ§amento no WhatsApp.',
            'Veja quais leads estÃ£o quentes.',
            'Controle agenda e sinais.',
            'Organize clientes e histÃ³rico.',
            'Acompanhe financeiro do estÃºdio.',
            'Use respostas rÃ¡pidas e IA para acelerar atendimento.',
            'Tenha relatÃ³rios para tomar decisÃµes melhores.',
        ] as $benefit) {
        ...
        echo '<div><h2>Quer testar no seu estÃºdio?</h2><p>Fale comigo e veja qual plano faz sentido para sua operaÃ§Ã£o.</p></div>';
    ...
    render_studio_shell('Inicio do CRM', 'Resumo operacional da instancia ' . (string)$user['studio_name'] . '.', 'home', function () use ($studio, $user) {
        ...
        if (is_array($whatsappStatusData) && !empty($whatsappStatusData['ok'])) {
            ...
            if (!empty($whatsappStatusData['phone'])) {
                $whatsappState .= ' Â· ' . preg_replace('/\D+/', '', (string)$whatsappStatusData['phone']);
            }
        }
        ...
        if ($staleAttentionLeadsCount > 0) {
            $alerts[] = [
                'title' => 'Leads sem atualizaÃ§Ã£o hÃ¡ mais de 24h',
                'description' => 'VocÃª tem ' . $staleAttentionLeadsCount . ' leads parados ou frios que merecem retorno.',
        ...
        if ($preScheduledNoSignalCount > 0) {
            $alerts[] = [
                'title' => 'PrÃ©-agendamentos sem sinal',
                'description' => 'HÃ¡ ' . $preScheduledNoSignalCount . ' prÃ©-agendamentos aguardando sinal.',
        ...
        if ($preScheduledOverlapCount > 0) {
            $alerts[] = [
                'title' => 'PrÃ©-agendamentos duplicados',
                'description' => 'HÃ¡ ' . $preScheduledOverlapCount . ' prÃ©-agendamentos no mesmo dia, horÃ¡rio e tatuador. Isso Ã© permitido, mas vale revisar.',
                'href' => app_url('studio_agenda'),
                'tone' => 'warn',
            ];
        }
        if (count($needsHumanConversations) > 0) {
            $alerts[] = [
                ...
                'description' => 'HÃ¡ ' . count($needsHumanConversations) . ' conversas pedindo atendimento humano. O alerta some quando alguÃ©m responder manualmente na conversa.',
        ...
        if (plan_allows('whatsapp')) {
            if (($whatsappStatusData['ok'] ?? false) && in_array((string)($whatsappStatusData['status'] ?? ''), ['disconnected', 'error'], true)) {
                $alerts[] = [
                    ...
                    'description' => 'Verifique a sessÃ£o do WhatsApp para continuar recebendo e respondendo conversas.',
            ...
            if ($pendingWhatsappCount > 0) {
                $alerts[] = [
                    ...
                    'description' => 'HÃ¡ ' . $pendingWhatsappCount . ' conversas que ainda aguardam retorno.',
                    'href' => app_url('studio_whatsapp'),
                    'tone' => 'warn',
                ];
            }
            ...
            $alerts[] = [
                'title' => 'WhatsApp indisponÃ­vel no plano atual',
                'description' => 'A integraÃ§Ã£o com WhatsApp aparece a partir do plano Profissional.',
        ...
        if (is_array($plan)) {
            $planLimits = [
                'max_users' => ['label' => 'usuÃ¡rios', 'count' => studio_user_count((int)$studio['id'])],
                ...
                'max_whatsapp_sessions' => ['label' => 'sessÃµes WhatsApp', 'count' => studio_whatsapp_session_count($studio)],
            ];
            foreach ($planLimits as $limitKey => $info) {
                ...
                if ($limitValue > 0 && $info['count'] >= (int)ceil($limitValue * 0.8) && $info['count'] < $limitValue) {
                    $alerts[] = [
                        'title' => 'Limite de ' . $info['label'] . ' prÃ³ximo',
                        'description' => 'Seu plano estÃ¡ prÃ³ximo do limite de ' . $info['label'] . '. Considere alterar para um plano superior.',
                        'href' => app_url('studio_settings'),
                        'tone' => 'warn',
                    ];
                }
        ...
        $homeDrilldowns = [
            'scheduled_month' => [
                ...
                'filters' => [
                    '7d' => 'PrÃ³ximos 7 dias',
                    '15d' => 'PrÃ³ximos 15 dias',
                    'month' => 'Este mÃªs',
                    'next_month' => 'MÃªs que vem',
                ],
                'rangeMap' => [
            ...
            'whatsapp_conversations' => [
                ...
                'summary' => plan_allows('whatsapp')
                    ...
                    : 'WhatsApp nÃ£o liberado no plano atual.',
                'type' => 'whatsapp',
            ...
            'open_value' => [
                ...
                'items' => [
                    ...
                    ['label' => 'Clientes cadastrados', 'value' => (string)$stats['customers'], 'detail' => 'Base total de clientes no estÃºdio.'],
                ],
            ],
            'appointments' => [
        ...
        echo '<p class="muted" style="margin:8px 0 0">Painel operacional do estÃºdio com agenda, leads, WhatsApp, financeiro e alertas do dia.</p>';
        echo '</div>';
        echo '</section>';
        ...
        foreach ([
            ...
            ['value' => format_money($scheduledToEndOfMonth), 'label' => 'Valor previsto no mÃªs', 'focus' => 'scheduled_month'],
            ['value' => format_money($stats['month_signals']), 'label' => 'Sinais recebidos no mÃªs', 'focus' => 'month_result'],
            ['value' => format_money($financeSummary['expenses_month'] ?? $stats['month_expenses']), 'label' => 'Despesas do mÃªs', 'focus' => 'month_result'],
            ['value' => format_money(($financeSummary['appointments_month'] ?? $stats['month_revenue']) - ($financeSummary['expenses_month'] ?? $stats['month_expenses'])), 'label' => 'Resultado estimado do mÃªs', 'focus' => 'month_result'],
        ] as $stat) {
...
if ($page === 'studio_customer') {
    ...
    render_studio_shell('Ficha do cliente', 'Historico completo de lead, WhatsApp e agenda.', 'customers', function () use ($studio) {
        ...
        echo '<div class="grid cols-3"><div class="field"><label>NÃºmero</label><input name="address_number" value="' . h($customer['address_number'] ?? '') . '"></div><div class="field"><label>Complemento</label><input name="address_complement" value="' . h($customer['address_complement'] ?? '') . '"></div><div class="field"><label>Bairro</label><input name="address_neighborhood" value="' . h($customer['address_neighborhood'] ?? '') . '"></div></div>';
        echo '<div class="grid cols-3"><div class="field"><label>Cidade</label><input name="address_city" value="' . h($customer['address_city'] ?? '') . '"></div><div class="field"><label>Estado</label><input name="address_state" value="' . h($customer['address_state'] ?? '') . '"></div><div class="field"><label>ReferÃªncia</label><input name="address_reference" value="' . h($customer['address_reference'] ?? '') . '"></div></div>';
        echo '<div class="grid cols-2"><div class="field"><label>Contato de emergÃªncia</label><input name="emergency_contact_name" value="' . h($customer['emergency_contact_name'] ?? '') . '"></div><div class="field"><label>Telefone de emergÃªncia</label><input name="emergency_contact_phone" value="' . h($customer['emergency_contact_phone'] ?? '') . '"></div></div>';
        echo '<div class="grid cols-2"><div class="field"><label>RegiÃ£o do corpo</label><input name="body_area" value="' . h($customer['body_area'] ?? '') . '"></div><div class="field"><label>Estilo de referÃªncia</label><input name="reference_style" value="' . h($customer['reference_style'] ?? '') . '"></div></div>';
        echo '<div class="grid cols-2"><div class="field"><label>JÃ¡ possui tatuagens?</label><textarea name="previous_tattoos">' . h($customer['previous_tattoos'] ?? '') . '</textarea></div><div class="field"><label>ResistÃªncia Ã  dor</label><select name="pain_tolerance">';
        render_options(['' => 'Selecionar', 'baixa' => 'Baixa', 'media' => 'MÃ©dia', 'alta' => 'Alta'], (string)($customer['pain_tolerance'] ?? ''));
        echo '</select></div></div>';
        ...
        echo '<div class="grid cols-2"><div class="field"><label>CondiÃ§Ãµes de saÃºde</label><textarea name="health_conditions">' . h($customer['health_conditions'] ?? '') . '</textarea></div><div class="field"><label>CondiÃ§Ãµes de pele</label><textarea name="skin_conditions">' . h($customer['skin_conditions'] ?? '') . '</textarea></div></div>';
        echo '<div class="grid cols-2"><div class="field"><label>HistÃ³rico de queloide</label><input name="keloid_history" value="' . h($customer['keloid_history'] ?? '') . '"></div><div class="field"><label>Uso de anticoagulantes</label><input name="anticoagulants" value="' . h($customer['anticoagulants'] ?? '') . '"></div></div>';
        echo '<div class="grid cols-2"><div class="field"><label>Diabetes</label><input name="diabetes" value="' . h($customer['diabetes'] ?? '') . '"></div><div class="field"><label>Problemas de cicatrizaÃ§Ã£o</label><input name="healing_issues" value="' . h($customer['healing_issues'] ?? '') . '"></div></div>';
        echo '<div class="grid cols-2"><label class="checkline"><input type="checkbox" name="data_processing_consent" value="1"' . (!empty($customer['data_processing_consent']) ? ' checked' : '') . '> Consentimento LGPD</label><label class="checkline"><input type="checkbox" name="marketing_opt_in" value="1"' . (!empty($customer['marketing_opt_in']) ? ' checked' : '') . '> Quer receber marketing</label><label class="checkline"><input type="checkbox" name="whatsapp_opt_in" value="1"' . (!empty($customer['whatsapp_opt_in']) ? ' checked' : '') . '> WhatsApp</label><label class="checkline"><input type="checkbox" name="sms_opt_in" value="1"' . (!empty($customer['sms_opt_in']) ? ' checked' : '') . '> SMS</label><label class="checkline"><input type="checkbox" name="email_opt_in" value="1"' . (!empty($customer['email_opt_in']) ? ' checked' : '') . '> Email</label><label class="checkline"><input type="checkbox" name="push_opt_in" value="1"' . (!empty($customer['push_opt_in']) ? ' checked' : '') . '> Push futuro</label><label class="checkline"><input type="checkbox" name="social_network_opt_in" value="1"' . (!empty($customer['social_network_opt_in']) ? ' checked' : '') . '> MarcaÃ§Ã£o em redes sociais</label><label class="checkline"><input type="checkbox" name="share_before_after_opt_in" value="1"' . (!empty($customer['share_before_after_opt_in']) ? ' checked' : '') . '> Antes/depois</label></div>';
...
if ($page === 'studio_leads') {
    $studio = require_studio();
    render_studio_shell('Funil de Leads', 'Acompanhe oportunidades, orÃ§amentos e agendamentos do estÃºdio.', 'leads', function () use ($studio) {
        $dbStatus = studio_db_status_for($studio);
        if (!$dbStatus['ok']) {
            render_studio_db_missing($studio, $dbStatus['error']);
            return;
        ...
        echo '<section class="panel dashboard-hero" style="margin-bottom:16px">';
        echo '<div class="dashboard-hero-copy">';
        echo '<p class="muted" style="margin:0 0 6px">Funil comercial do estÃºdio</p>';
        ...
        echo '<p class="muted" style="margin:8px 0 0">Acompanhe oportunidades, orÃ§amentos e agendamentos do estÃºdio.</p>';
        echo '</div>';
        ...
        foreach ([0 => 'Qualquer nota', 4 => 'Nota mÃ­nima 4', 7 => 'Nota mÃ­nima 7', 8 => 'Quentes (8+)'] as $key => $label) {
        ...
        foreach ([
            ...
            ['value' => (string)count($preScheduledLeads), 'label' => 'PrÃ©-agendados'],
        ...
        echo '<div class="actions" style="justify-content:space-between;align-items:flex-start"><div><h2>Funil de Leads</h2><p class="muted">Etapas ordenadas, total por coluna e cartÃµes com aÃ§Ã£o comercial.</p></div><span class="badge">' . h((string)count($openLeads)) . ' leads abertos</span></div>';
        if (!$allLeads) {
            if ($initialLeadCount === 0) {
                echo '<div class="drilldown-empty"><strong>Nenhum lead cadastrado ainda.</strong><div class="muted">Crie o primeiro lead para comeÃ§ar a operar o funil.</div><a class="btn" href="' . h(app_url('studio_lead', ['id' => 0])) . '">Criar primeiro lead</a></div>';
            } else {
                echo '<div class="drilldown-empty"><strong>Nenhum lead encontrado para este filtro.</strong><div class="muted">Tente limpar os filtros ou buscar outra combinaÃ§Ã£o.</div><a class="btn" href="' . h(app_url('studio_leads')) . '">Limpar filtros</a></div>';
            }
        }
        ...
        if (!$hotLeads && !$staleLeads) {
            ...
            foreach ($attentionCards as $lead) {
                ...
                echo '<span class="muted">' . h(($lead['status'] ?: '-') . ' Â· ' . ($lead['pipeline_stage'] ?: '-') . ' Â· ' . ($lead['source'] ?: 'Sem origem')) . '</span>';
                echo '<span>' . h(($lead['interest'] ?: 'Sem interesse descrito.') . ' Â· ' . format_money($lead['estimated_value'] ?? 0)) . '</span>';
        ...
        echo '<div class="panel"><div class="actions" style="justify-content:space-between"><h2>Filtro rÃ¡pido de etapas</h2><span class="badge">Status comercial</span></div>';
        echo '<div class="stack-list">';
        foreach ($board as $stageName => $column) {
            ...
            echo '<a class="activity-card" href="' . h($href) . '"><strong>' . h($stageName) . '</strong><span class="muted">' . h($count . ' leads Â· ' . format_money($value)) . '</span><span>Clique para focar nesta etapa.</span></a>';
        }
        echo '</div></div>';
        echo '</section>';
    }, $flash);
...
if ($page === 'studio_lead') {
    ...
    render_studio_shell('Detalhe do lead', 'Historico, funil e proximas acoes.', 'leads', function () use ($studio) {
        ...
        if ($leadId <= 0) {
            ...
            echo '<div class="actions" style="justify-content:space-between;align-items:flex-start"><div><h2>Novo lead</h2><p class="muted">Crie uma oportunidade nova para o funil do estÃºdio.</p></div><span class="badge">Cadastro</span></div>';
            ...
            echo '</select></div>';
            echo '<div class="field"><label>Interesse</label><input name="interest" placeholder="Ex.: tatuagem fina no antebraÃ§o"></div>';
            echo '<div class="grid cols-3"><div class="field"><label>Status</label><select name="status">';
            ...
            echo '<div class="grid cols-2"><div class="field"><label>Valor estimado</label><input name="estimated_value" value="0"></div><div class="field"><label>Origem</label><input name="source" placeholder="Instagram, WhatsApp, indicaÃ§Ã£o..."></div></div>';
            echo '<div class="field"><label>Tatuador / responsÃ¡vel</label><select name="artist_id">';
            render_artist_options($artists, default_artist_id($studio) ?? 0);
            echo '</select></div>';
            echo '<button class="btn" type="submit">Salvar lead</button>';
            echo '</form>';
            echo '<div class="panel soft"><p class="muted">Dica</p><h3 style="margin-top:0">Depois de salvo, o lead jÃ¡ entra no funil e pode ser movido entre etapas com os botÃµes do card.</h3></div>';
            echo '</section>';
            return;
        }
        $lead = studio_find_lead($studio, $leadId);
        ...
        echo '<div class="actions" style="margin-top:14px;gap:8px;flex-wrap:wrap"><a class="btn secondary" href="' . h($publicUpdateUrl) . '" target="_blank" rel="noopener">Abrir formulÃ¡rio</a><button type="button" class="btn secondary" data-copy-link="' . h($publicUpdateUrl) . '">Copiar link</button><a class="btn secondary" href="https://wa.me/?text=' . h(rawurlencode($shareMessage)) . '" target="_blank" rel="noopener">Enviar no WhatsApp</a></div>';
        echo '</div>';
...
if ($page === 'studio_agenda') {
    ...
    render_studio_shell('Agenda', 'Calendario, tatuadores e proximos atendimentos.', 'agenda', function () use ($studio) {
        ...
        echo '<button type="button" class="btn secondary" id="openFreeSlotsButton">PrÃ³ximos horÃ¡rios livres</button>';
        echo '</div>';
        if (is_array($importPreview)) {
            ...
            echo '<div><h2>Revisar importacao Google Agenda</h2><p class="muted">Escolha o que importar, ajuste os campos e confirme apenas o que realmente faz sentido para o estÃºdio.</p></div>';
            ...
            echo '<article class="alert-card"><span class="badge warn">' . h((string)($analysis['duplicates'] ?? 0)) . '</span><p><strong>Possiveis duplicados</strong></p><p class="muted">Eventos ja importados ou muito parecidos com importaÃ§Ãµes anteriores.</p></article>';
            ...
            foreach ($candidates as $candidate) {
                ...
                if ($conflicts) {
                    ...
                    foreach ($conflicts as $conflict) {
                        ...
                        echo '<div class="muted">' . h($conflictName) . ' Â· ' . h($conflictArtist) . ' Â· ' . h((string)($conflict['status'] ?? '')) . '</div>';
                        echo '</div>';
                    }
                    echo '</div>';
                ...
                echo '<label>InÃ­cio<input type="time" name="items[' . h($uid) . '][start_time]" value="' . h(substr((string)($candidate['start_time'] ?? ''), 0, 5)) . '"></label>';
                ...
                echo '</div>';
                echo '<label style="display:block;margin-top:12px">Interesse/observaÃ§Ãµes<textarea name="items[' . h($uid) . '][interest]" rows="2">' . h(trim($notes !== '' ? $notes . "\n" : '') . trim($description)) . '</textarea></label>';
            ...
            if (!$candidates) {
                echo '<div class="alert-card"><p><strong>Nenhum candidato encontrado</strong></p><p class="muted">Esse arquivo nÃ£o gerou eventos aptos para importaÃ§Ã£o.</p></div>';
            }
            echo '</div>';
            echo '<div class="actions" style="justify-content:space-between;margin-top:16px;gap:12px;flex-wrap:wrap">';
            echo '<p class="muted">Itens com conflito ficam destacados. Por padrÃ£o eles nÃ£o sÃ£o importados, a menos que vocÃª marque a opÃ§Ã£o de manter mesmo assim.</p>';
        ...
        echo '<article class="alert-card"><span class="badge warn">' . h((string)$preScheduledNoSignalCount) . '</span><p><strong>PrÃ©-agendamentos sem sinal</strong></p><p class="muted">HÃ¡ prÃ©-agendamentos aguardando confirmaÃ§Ã£o financeira.</p></article>';
        echo '<article class="alert-card"><span class="badge ok">' . h((string)count($todayAppointments)) . '</span><p><strong>Agendamentos de hoje</strong></p><p class="muted">Confira a ocupaÃ§Ã£o do dia atual sem sair da agenda.</p></article>';
        echo '<article class="alert-card"><span class="badge danger">' . h((string)$missingArtistCount) . '</span><p><strong>Sem tatuador definido</strong></p><p class="muted">Agendamentos sem tatuador precisam de revisÃ£o.</p></article>';
        echo '<article class="alert-card"><span class="badge warn">' . h((string)$missingContactCount) . '</span><p><strong>Sem cliente/lead vinculado</strong></p><p class="muted">Esses agendamentos merecem vÃ­nculo para evitar perda de contexto.</p></article>';
        echo '</div>';
        echo '<div id="freeSlotsModal" class="crm-modal hidden"><div class="crm-modal-panel" style="max-width:min(96vw,1100px)"><div class="crm-panel-header"><div><h3 class="crm-panel-title">PrÃ³ximos horÃ¡rios livres</h3><p class="muted" style="margin:4px 0 0">Primeiras janelas livres encontradas na agenda.</p></div><button type="button" id="closeFreeSlotsModal" class="crm-button crm-icon-button"><i class="fa-solid fa-xmark"></i></button></div><div class="p-4"><div class="stack-list">';
        if (!$nextAvailableSlots) {
            echo '<p class="muted">NÃ£o foi possÃ­vel calcular horÃ¡rios livres neste recorte.</p>';
        } else {
            foreach (array_slice($nextAvailableSlots, 0, 12) as $slot) {
                $href = app_url('studio_agenda', ['date' => (string)$slot['date']]) . '#appointment-form';
                echo '<a class="activity-card" href="' . h($href) . '"><strong>' . h((string)$slot['label']) . '</strong><span class="muted">' . h(implode(' Â· ', array_slice($slot['free_slots'] ?? [], 0, 4))) . '</span><span>' . h((string)count($slot['free_slots'] ?? [])) . ' horÃ¡rios livres</span></a>';
            }
        }
...
if ($page === 'studio_whatsapp') {
    ...
    render_studio_shell('WhatsApp', 'Central de conversas e sessao Baileys deste estudio.', 'whatsapp', function () use ($studio) {
        ...
        echo '<a class="panel quick-action-card" href="' . h(app_url('studio_whatsapp', ['mode' => 'bot'])) . '"><strong>' . h($summary['bot']) . '</strong><span>Em modo IA</span><small>Ver conversas automÃ¡ticas</small></a>';
        echo '<a class="panel quick-action-card" href="' . h(app_url('studio_whatsapp', ['needs_human' => 1])) . '"><strong>' . h($summary['needs_human']) . '</strong><span>Pedindo humano</span><small>Atendimentos que pedem atenÃ§Ã£o</small></a>';
        echo '<a class="panel quick-action-card" href="#wa-session-panel"><strong>' . h($serviceStateLabel) . '</strong><span>Sessao WhatsApp</span><small>Ver conexÃ£o e pareamento</small></a>';
        echo '</section>';
...
if ($page === 'studio_whatsapp_conversation') {
    ...
    render_studio_shell('Conversa WhatsApp', 'Historico, atendimento e envio direto.', 'whatsapp', function () use ($studio) {
        ...
        $availabilityRanges = [
            ...
            'month' => ['label' => 'Este mÃªs', 'days' => max(1, (int)$monthEnd->diff($availabilityStart)->days + 1)],
            'next_month' => ['label' => 'MÃªs que vem', 'start' => $availabilityStart->modify('first day of next month'), 'days' => (int)$availabilityStart->modify('first day of next month')->format('t')],
        ...
        foreach (['ðŸ˜€','ðŸ‘','ðŸ™','â¤ï¸','ðŸ˜‚','ðŸ”¥','ðŸŽ¯','âœ…'] as $emoji) {
        ...
        echo '<div class="crm-panel-header"><div><h3 class="crm-panel-title">Ferramentas da conversa</h3><p class="muted" style="margin:4px 0 0">Cadastro, IA e respostas rÃ¡pidas em um sÃ³ lugar.</p></div><button type="button" id="closeConversationToolsOverlay" class="crm-button crm-icon-button"><i class="fa-solid fa-xmark"></i></button></div>';
        ...
        echo '<summary>SugestÃµes da IA</summary>';
        if (!empty($assistantInsights['suggested_name']) || !empty($assistantInsights['suggested_interest']) || !empty($assistantInsights['suggested_notes']) || !empty($assistantInsights['schedule_reason'])) {
            ...
            echo '<p class="muted">Nenhuma sugestÃ£o clara detectada ainda.</p>';
        }
        echo '</details>';
...
if ($page === 'studio_people') {
    ...
    render_studio_shell('Pessoas', 'Clientes e leads num unico lugar.', 'people', function () use ($studio) {
        ...
        echo '<a class="panel dashboard-stat" href="' . h(app_url('studio_whatsapp')) . '"><p class="metric">' . h((string)studio_whatsapp_summary($studio)['total']) . '</p><p class="muted">Conversas WhatsApp</p><span class="muted">Ver integraÃ§Ãµes</span></a>';
        echo '</section>';
        echo '<section class="grid cols-2" style="margin-top:16px">';
        if ($view !== 'customers') {
...
if ($page === 'studio_reports') {
    ...
    render_studio_shell('Relatorios', 'Leitura gerencial do funil, agenda e financeiro.', 'reports', function () use ($studio) {
        ...
        if ($staleLeadsRows) {
            $alerts[] = [
                'title' => 'Leads abertos sem atualizaÃ§Ã£o hÃ¡ mais de 24h',
                'count' => count($staleLeadsRows),
                'tone' => 'warn',
                'items' => array_map(static function (array $lead): array {
                    return [
                        'label' => ($lead['name'] ?: $lead['phone'] ?: 'Lead sem nome'),
                        'detail' => ($lead['pipeline_stage'] ?: 'Sem etapa') . ' Â· nota ' . ((string)($lead['lead_score'] ?? 0)) . '/10',
                        'href' => app_url('studio_lead', ['id' => (int)$lead['id']]),
                    ];
                }, $staleLeadsRows),
            ];
        ...
        if ($highScoreUnscheduled) {
            $alerts[] = [
                'title' => 'Leads com score alto ainda nÃ£o agendados',
                'count' => count($highScoreUnscheduled),
                'tone' => 'ok',
                'items' => array_map(static function (array $lead): array {
                    return [
                        'label' => ($lead['name'] ?: $lead['phone'] ?: 'Lead sem nome'),
                        'detail' => 'Score ' . ((string)($lead['lead_score'] ?? 0)) . '/10 Â· ' . format_money($lead['estimated_value'] ?? 0),
                        'href' => app_url('studio_lead', ['id' => (int)$lead['id']]),
                    ];
...
'detail' => format_date_pt((string)$appointment['appointment_date']) . ' Ã s ' . substr((string)$appointment['start_time'], 0, 5) . ' Â· ' . format_money(appointment_display_amount($appointment['value'] ?? 0)),
                        'href' => $href,
                    ];
...
'detail' => format_date_pt((string)$appointment['appointment_date']) . ' Â· ' . substr((string)$appointment['start_time'], 0, 5) . ' Â· ' . (string)($appointment['status'] ?? '-') . ' Â· ' . format_money(appointment_display_amount($appointment['value'] ?? 0)) . ' Â· sinal ' . format_money(appointment_display_amount($appointment['deposit_value'] ?? 0)),
                        'href' => $href,
                    ];
                }, $todayAppointments),
            ];
        ...
        if (!empty($confirmationAutomation['canceled'])) {
            $alerts[] = [
                'title' => 'ConfirmaÃ§Ãµes vencidas canceladas',
                'count' => (int)$confirmationAutomation['canceled'],
                'tone' => 'danger',
                'items' => array_map(static function (array $event): array {
                    return [
                        'label' => 'Agendamento cancelado por falta de confirmaÃ§Ã£o',
                        'detail' => 'A janela de confirmaÃ§Ã£o expirou sem resposta do cliente.',
                        'href' => app_url('studio_agenda'),
                    ];
        ...
        if ($alerts) {
            echo '<section class="panel" style="margin-bottom:16px"><div class="actions" style="justify-content:space-between;align-items:flex-start"><div><h2>Alertas operacionais</h2><p class="muted">Sinais rÃ¡pidos do que precisa de aÃ§Ã£o agora.</p></div><span class="badge">OperaÃ§Ã£o</span></div>';
            ...
            echo '<div class="alert-card"><strong>Despesas do mÃªs</strong><p class="metric" style="margin:8px 0 0">' . h(format_money($monthExpenses)) . '</p><p class="muted">Total de despesas registradas no perÃ­odo atual.</p><a class="btn tiny secondary" href="' . h(app_url('studio_finance')) . '">Abrir financeiro</a></div>';
            echo '</div></section>';
        }
        ...
        if (plan_allows('advanced_reports')) {
            ...
            echo '<section class="panel" style="margin-top:16px">';
            echo '<div class="actions" style="justify-content:space-between;align-items:flex-start;gap:12px"><div><h2>Tabela dinÃ¢mica</h2><p class="muted">Monte cruzamentos por arrastar campos entre linhas, colunas, medidas e filtros.</p></div><span class="badge">AnÃ¡lise</span></div>';
            ...
            echo '<section class="panel" style="margin-top:16px"><div class="actions" style="justify-content:space-between;align-items:flex-start;gap:12px"><div><h2>Tabela dinÃ¢mica</h2><p class="muted">Este bloco avanÃ§ado fica disponÃ­vel a partir do plano Profissional ou AvanÃ§ado.</p></div><span class="badge warn">Bloqueado</span></div>';
            echo '<p class="muted">O restante dos relatÃ³rios continua disponÃ­vel normalmente. Para liberar a pivot, considere alterar para um plano superior.</p>';
            echo '</section>';
        }
        echo '<section class="grid cols-2" style="margin-top:16px">';
        ...
        echo '</div><div class="panel"><h2>Agenda por mÃªs</h2>';
...
if ($page === 'studio_data_assistant') {
    ...
    render_studio_shell('Assistente IA de dados', 'Perguntas internas sobre CRM, agenda, WhatsApp e financeiro.', 'assistant', function () use ($studio) {
        ...
        if (!plan_allows('ai_data_assistant')) {
            echo '<section class="panel"><div class="actions" style="justify-content:space-between;align-items:flex-start"><div><h2>Assistente IA de dados</h2><p class="muted">Os recursos de IA estÃ£o disponÃ­veis no plano AvanÃ§ado.</p></div><span class="badge warn">Bloqueado</span></div><p class="muted">Esse assistente continua somente leitura e nÃ£o altera dados. Para usar as respostas por IA, altere para um plano superior.</p></section>';
            return;
        }
...
if ($page === 'studio_settings') {
    ...
    render_studio_shell('Configuracoes do estudio', 'Regras comerciais e preparacao dos modulos de IA/WhatsApp.', 'settings', function () use ($studio) {
        ...
        echo '<div class="actions" style="justify-content:space-between;align-items:center"><h3 style="margin:0">EstÃºdio</h3><a class="btn tiny secondary" href="#topo-configuracoes">Voltar ao topo</a></div>';
        echo '<div class="grid cols-2">';
        ...
        echo '<div class="field"><label>WhatsApp habilitado neste estudio</label><label class="checkline"><input type="checkbox" name="whatsapp_enabled" value="1" ' . (!empty($settings['whatsapp_enabled']) ? 'checked' : '') . '> Ativar/Desativar integraÃ§Ã£o</label></div>';
        echo '</div>';
        echo '</div>';
        ...
        $dayOptions = ['1' => 'Segunda', '2' => 'TerÃ§a', '3' => 'Quarta', '4' => 'Quinta', '5' => 'Sexta', '6' => 'SÃ¡bado', '7' => 'Domingo'];
        ...
        echo '<div class="field"><label>Dias da semana disponÃ­veis</label><div class="weekday-picker">';
        ...
        echo '</div><small class="muted">Selecione os dias em que o estudio atende. O padrÃ£o vem de segunda a sexta.</small></div>';
        echo '<div class="field"><label>HorÃ¡rios disponÃ­veis</label><input name="appointment_time_slots" value="' . h($settings['appointment_time_slots'] ?? '10:00,15:00') . '" placeholder="10:00,15:00"><small class="muted">Separe por vÃ­rgula. Ex: 10:00,15:00</small></div>';
        echo '<div class="field"><label>Valor da pomada</label><input name="pomada_unit_price" value="' . h(number_format($pomadaUnitPrice, 2, ',', '.')) . '" placeholder="100,00"><small class="muted">Este valor vale sÃ³ para novos agendamentos. Os antigos mantÃªm o preÃ§o salvo neles.</small></div>';
        echo '</div>';
        echo '<div class="grid cols-3">';
        echo '<div class="field"><label>DuraÃ§Ã£o do atendimento</label><div class="duration-picker">';
        ...
        echo '</select></label>';
        echo '</div><small class="muted">O fim serÃ¡ calculado automaticamente. Ex: 5 horas = 10:00 atÃ© 15:00.</small></div>';
        echo '</div>';
        echo '<div class="field"><label>Mensagem quando a vaga for tomada por um confirmado</label><textarea name="appointment_overwrite_message" placeholder="Oi {{name}}, sua vaga do dia {{date}} Ã s {{start_time}} foi ocupada por outro agendamento confirmado com sinal pago. Escolha outro horÃ¡rio e envie o sinal para garantir a nova vaga.">' . h($settings['appointment_overwrite_message'] ?? 'Oi {{name}}, sua vaga do dia {{date}} Ã s {{start_time}} foi ocupada por outro agendamento confirmado com sinal pago. Escolha outro horÃ¡rio e envie o sinal para garantir a nova vaga.') . '</textarea><small class="muted">Aceita variÃ¡veis: {{name}}, {{date}}, {{start_time}}, {{end_time}}, {{new_date}}, {{new_start_time}}, {{new_end_time}}, {{studio_name}}, {{reason}}</small></div>';
        echo '<div class="field"><label>Mensagem de confirmaÃ§Ã£o do agendamento</label><textarea name="appointment_confirmation_message" placeholder="Oi {{name}}! Sua sessÃ£o estÃ¡ confirmada para {{date}} Ã s {{start_time}}. Me responde com sim para confirmar, ou avisa se precisar cancelar/alterar.">' . h($settings['appointment_confirmation_message'] ?? 'Oi {{name}}! Sua sessÃ£o estÃ¡ confirmada para {{date}} Ã s {{start_time}}. Me responde com sim para confirmar, ou avisa se precisar cancelar/alterar.') . '</textarea><small class="muted">Aceita variÃ¡veis: {{name}}, {{date}}, {{start_time}}, {{end_time}}, {{studio_name}}, {{reason}}</small></div>';
        echo '</div>';
        ...
        echo '<div class="field"><label>Chave da OpenAI</label><input name="openai_api_key" type="password" value="' . h($settings['openai_api_key'] ?? '') . '" placeholder="sk-..."><small class="muted">Preencha sÃ³ se usar OpenAI.</small></div>';
        echo '</div>';
        echo '<div class="grid cols-2">';
        echo '<div class="field"><label>Modelo da IA no WhatsApp</label><input name="openai_model" value="' . h($settings['openai_model'] ?? 'qwen3:4b') . '" placeholder="qwen3:4b"><small class="muted">No Ollama, esse campo tambÃ©m define o modelo local.</small></div>';
        echo '<div class="settings-switch-grid"><label class="checkline"><input type="checkbox" name="ai_enabled" value="1" ' . (!empty($settings['ai_enabled']) ? 'checked' : '') . '> IA pode responder conversas marcadas como IA</label><label class="checkline"><input type="checkbox" name="assistant_autofill_enabled" value="1" ' . (!empty($settings['assistant_autofill_enabled']) ? 'checked' : '') . '> Assistente preencher sugestÃµes automaticamente nas conversas</label><label class="checkline"><input type="checkbox" name="whatsapp_enabled" value="1" ' . (!empty($settings['whatsapp_enabled']) ? 'checked' : '') . '> WhatsApp/Baileys ativo neste estudio</label></div>';
        echo '</div>';
        echo '</div>';
        ...
        echo '<div class="field"><label>Regras e informaÃ§Ãµes para IA</label><textarea name="business_rules" placeholder="Exemplo: EstÃºdio aberto de terÃ§a a sÃ¡bado. Dois tatuadores. Responder sempre em portuguÃªs do Brasil. Quando o cliente pedir agendamento, considerar sinal obrigatÃ³rio. NÃ£o inventar preÃ§o. Se faltar informaÃ§Ã£o, perguntar sÃ³ uma coisa por vez. Priorize datas, horÃ¡rios e referÃªncias reais do estÃºdio.">' . h($settings['business_rules'] ?? $studio['business_rules'] ?? '') . '</textarea><small class="muted">Esse texto entra no contexto da IA. Aqui vale escrever regras reais do estÃºdio, tom de atendimento, limites e informaÃ§Ãµes que a IA deve respeitar sempre.</small></div>';
        echo '<div class="field"><label>Texto-base da IA para WhatsApp</label><textarea name="ai_whatsapp_prompt" placeholder="VocÃª Ã© o assistente do estÃºdio...">' . h($settings['ai_whatsapp_prompt'] ?? '') . '</textarea><small class="muted">Se vazio, o sistema usa um texto-base em portuguÃªs jÃ¡ pronto.</small></div>';
        echo '</div>';

        echo '<div class="actions" style="justify-content:space-between;align-items:center;margin-top:12px"><span class="muted">Salvar continua aplicando as regras no banco do estudio.</span><button class="btn" type="submit" form="studioSettingsForm">Salvar configuraÃ§Ãµes</button></div>';
        echo '</form>';
        ...
        echo '<div class="panel soft">';
        echo '<p class="muted">Cadastre e gerencie os tatuadores que aparecem na agenda e nos agendamentos do estÃºdio.</p>';
        ...
        echo '<div class="actions" style="justify-content:space-between;align-items:center"><h3 style="margin:0">Respostas rÃ¡pidas</h3><a class="btn tiny secondary" href="#topo-configuracoes">Voltar ao topo</a></div>';
        echo '<div class="actions" style="justify-content:space-between"><div><p class="muted">Esses textos prontos ficam disponÃ­veis no atendimento e continuam editÃ¡veis por aqui.</p></div><a class="btn secondary" href="' . h(app_url('studio_quick_replies')) . '">Abrir biblioteca</a></div>';
    ...
    render_app_shell('Estudios', 'Cadastros e isolamento de bancos.', 'studios', function () use ($studios) {
        ...
        if (!$studios) {
            ...
            foreach ($studios as $studio) {
                ...
                echo '<tr>';
                echo '<td><strong>' . h($studio['name']) . '</strong><br><span class="muted">' . h($studio['slug']) . ' Â· ' . h(commercial_plan_display_name($plan, (string)($studio['plan_name'] ?? ''))) . '</span></td>';
        ...
        if (!$plans) {
            ...
            foreach ($plans as $plan) {
                ...
                foreach ([
                    ...
                    'AutomaÃ§Ãµes' => !empty($plan['allow_automations']),
                    'Multi-estudio' => !empty($plan['allow_multi_studio']),
                    'IntegraÃ§Ãµes' => !empty($plan['allow_external_integrations']),
                ] as $label => $enabled) {
...
if ($page === 'studio') {
    ...
    render_app_shell((string)$studio['name'], 'Instancia alpha do CRM deste estudio.', 'studios', function () use ($studio) {
        ...
        if ($plan) {
            echo '<span class="muted">' . h(format_money((float)$plan['monthly_price'])) . '/mes Â· ' . h(format_money((float)$plan['annual_price'])) . '/ano</span>';
        }
        echo '</div>';
        echo '</section>';
...
function render_studio_form(?array $studio): void
{
    ...
    if ($plans) {
        ...
        foreach ($plans as $plan) {
            ...
            echo '<option value="' . h($plan['id']) . '" ' . $selected . '>' . h($plan['name'] . ' Â· ' . format_money((float)$plan['monthly_price']) . '/mes') . '</option>';
        }
        echo '</select>';
    } else {
...
function render_commercial_plan_form(?array $plan): void
{
    ...
    echo '<div class="field"><label>Limite de estÃºdios</label><input type="number" min="0" name="studio_limit" value="' . h($plan['studio_limit'] ?? '') . '" placeholder="0 = ilimitado"></div>';
    echo '<div class="field"><label>Limite de usuÃ¡rios</label><input type="number" min="0" name="user_limit" value="' . h($plan['user_limit'] ?? '') . '" placeholder="0 = ilimitado"></div>';
    ...
    echo '<div class="field"><label>Limite de sessÃµes WhatsApp</label><input type="number" min="0" name="whatsapp_session_limit" value="' . h($plan['whatsapp_session_limit'] ?? '') . '" placeholder="0 = sem WhatsApp"></div>';
    echo '</div>';
...
function render_calendar_week(array $appointments, DateTimeImmutable $focus, ?float $pomadaUnit = null): void
{
    ...
    for ($i = 0; $i < 7; $i++) {
        ...
        echo '<div class="calendar-day-summary"><small>' . h(count($dayAppointments) . ' agendamentos Â· ' . format_money($dayValue)) . '</small></div>';
        foreach ($dayAppointments as $appointment) {
            render_calendar_event($appointment);
        }
...
function render_pipeline_card(array $lead, array $stageNames): void
{
    ...
    if ($isScheduled) {
        echo '<span class="badge warn">' . h($status === 'agendado' ? 'Agendado' : 'PrÃ©-agendado') . '</span>';
    }
    if ($artistName !== '') {
        echo '<span class="badge">' . h($artistName) . '</span>';
    }
    if ($isStale) {
        echo '<span class="badge warn">parado hÃ¡ mais de 24h</span>';
    }
    echo '</div>';
...
function studio_relative_time_label(?string $value): string
{
    ...
    try {
        ...
        foreach ($units as [$prop, $threshold, $singular, $plural]) {
            ...
            return $past ? 'hÃ¡ ' . $amount . ' ' . $label : 'em ' . $amount . ' ' . $label;
        }
        return $past ? 'hÃ¡ instantes' : 'agora';
    } catch (Throwable) {
        return $value;
    }
}
...
function render_whatsapp_table(array $conversations): void
{
    ...
    echo '<table class="table whatsapp-conversations-table"><thead><tr><th>Contato</th><th>Ãšltima mensagem</th><th>Modo</th><th>VÃ­nculo</th><th>SituaÃ§Ã£o</th><th>AÃ§Ãµes</th></tr></thead><tbody>';
    foreach ($conversations as $conversation) {
        ...
        $linkedLabel = !empty($conversation['customer_id']) ? 'Cliente vinculado' : (!empty($conversation['lead_id']) ? 'Lead vinculado' : 'Sem vÃ­nculo');
        $linkBadgeClass = $linkedLabel === 'Sem vÃ­nculo' ? 'warn' : '';
        $statusBadges = [];
...
function render_chat_messages(array $messages): void
{
    ...
    foreach ($messages as $message) {
        ...
        if ($mediaUrl !== '') {
            if ($kind === 'image') {
                echo '<button type="button" class="chat-media-thumb" onclick="window.openMediaOverlay && window.openMediaOverlay(this.dataset.mediaSrc, this.dataset.mediaTitle, this.dataset.mediaKind)" data-media-src="' . h($mediaUrl) . '" data-media-title="' . h($mediaName ?: 'mÃ­dia') . '" data-media-kind="image" aria-label="Abrir imagem em tamanho grande"><img src="' . h($mediaUrl) . '" alt="' . h($mediaName ?: 'mÃ­dia') . '" style="max-width:260px;max-height:220px;border-radius:8px"></button>';
            } elseif ($kind === 'video') {
