...
if ($page === 'studio_settings') {
    ...
    render_studio_shell('Configurações do estúdio', 'Regras comerciais e preparação dos módulos de IA/WhatsApp.', 'settings', function () use ($studio) {
        ...
        echo '</form>';
        echo '<script>(function(){const modal=document.getElementById("settingsOverlay");const body=document.getElementById("settingsOverlayBody");const title=document.getElementById("settingsOverlayTitle");const summary=document.getElementById("settingsOverlaySummary");const closeBtn=document.getElementById("closeSettingsOverlay");const settingsForm=document.getElementById("studioSettingsForm");const sourceMap={studio:{title:"Estúdio",summary:"Dados base e integração",source:"settingsSourceStudio"},agenda:{title:"Agenda",summary:"Regras de horário e duração",source:"settingsSourceAgenda"},whatsapp:{title:"WhatsApp",summary:"Entrada e comportamento",source:"settingsSourceWhatsapp"},ia:{title:"IA",summary:"Modelo, chave e automação",source:"settingsSourceIa"},meta_ads:{title:"Meta Ads",summary:"Credenciais, IDs e diagnóstico",source:"settingsSourceMetaAds"},quick_replies:{title:"Respostas rápidas",summary:"Biblioteca do atendimento",source:"settingsSourceQuickReplies"},rules:{title:"Regras comerciais",summary:"Contexto para a IA",source:"settingsSourceRules"}};function syncOverlayToSource(){if(!settingsForm||!body)return;body.querySelectorAll("input,textarea,select").forEach((field)=>{const name=field.getAttribute("name");if(!name)return;const sourceField=Array.from(settingsForm.querySelectorAll("input,textarea,select")).find((candidate)=>candidate.getAttribute("name")===name);if(!sourceField)return;if(sourceField.type==="checkbox"||sourceField.type==="radio"){sourceField.checked=field.checked;}else{sourceField.value=field.value;}});}function bindLiveSync(){if(!body)return;body.querySelectorAll("input,textarea,select").forEach((field)=>{field.addEventListener("input",syncOverlayToSource);field.addEventListener("change",syncOverlayToSource);});}function disableTemplateFields(source){source.querySelectorAll("input,textarea,select").forEach((field)=>{field.disabled=true;});}function openOverlay(key){const config=sourceMap[key]||sourceMap.studio;const source=document.getElementById(config.source);if(!modal||!body||!title||!summary||!source)return;title.textContent=config.title;summary.textContent=config.summary;body.innerHTML=source.innerHTML;body.querySelectorAll("input,textarea,select").forEach((field)=>{field.disabled=false;});disableTemplateFields(source);bindLiveSync();syncOverlayToSource();modal.classList.remove("hidden");}if(settingsForm){settingsForm.addEventListener("submit",()=>{syncOverlayToSource();});}document.querySelectorAll("[data-settings-overlay]").forEach((button)=>{button.addEventListener("click",()=>openOverlay(button.getAttribute("data-settings-overlay")||"studio"));});if(closeBtn) closeBtn.addEventListener("click",()=>modal.classList.add("hidden"));if(modal) modal.addEventListener("click",(event)=>{if(event.target===modal) modal.classList.add("hidden");});document.addEventListener("keydown",(event)=>{if(event.key==="Escape"&&modal) modal.classList.add("hidden");});})();</script>';
