...
if ($page === 'studio_whatsapp_conversation') {
    ...
    render_studio_shell('Conversa WhatsApp', 'Historico, atendimento e envio direto.', 'whatsapp', function () use ($studio) {
        ...
        echo 'document.querySelectorAll("[data-mode-toggle]").forEach(button => button.addEventListener("click", async () => { try { await postConversationUpdate({ action: "update_whatsapp_profile", attendance_mode: button.dataset.modeToggle === "bot" ? "bot" : "human" }, "Nao foi possivel atualizar o atendimento."); location.reload(); } catch (error) { alert(error.message || "Nao foi possivel atualizar o atendimento."); } }));';
        echo 'document.querySelectorAll("[data-status-set]").forEach(button => button.addEventListener("click", async () => { try { await postConversationUpdate({ action: "update_whatsapp_profile", status: button.dataset.statusSet || "novo" }, "Nao foi possivel atualizar o status."); location.reload(); } catch (error) { alert(error.message || "Nao foi possivel atualizar o status."); } }));';
