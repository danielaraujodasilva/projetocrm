...
if ($page === 'lead_public_update') {
    ...
    render_public_page('Atualizar cadastro', 'Complete seus dados para agilizar o atendimento.', function () use ($lead, $leadId, $token, $customerSeed, $missingFields) {
        $checked = static fn(string $field): string => empty($customerSeed) || empty($customerSeed[$field]) ? ' checked' : ' checked';
        $value = static fn(string $field, string $fallback = ''): string => (string)($customerSeed[$field] ?? $fallback);
        echo '<section class="panel" style="max-width:1040px;margin:0 auto;background:linear-gradient(180deg,rgba(31,111,120,0.08),rgba(255,255,255,0.98) 32%,rgba(31,111,120,0.02));border:1px solid rgba(31,111,120,0.16);box-shadow:0 18px 50px rgba(0,0,0,0.10)">';
        echo '<div class="actions" style="justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap">';
        echo '<div style="flex:1;min-width:280px"><span class="badge ok">Cadastro rápido</span><h1 style="margin:10px 0 8px;font-size:clamp(28px,3vw,42px)">Complete sua ficha</h1><p class="muted" style="max-width:68ch;margin:0;line-height:1.65">A gente deixou essa etapa mais leve para você. Leva poucos minutos, economiza tempo depois e já organiza seu atendimento com mais carinho e segurança.</p></div>';
        echo '<div class="panel soft" style="min-width:250px;max-width:320px"><strong>O que você ganha ao preencher</strong><div class="stack" style="margin-top:10px;gap:8px"><div class="badge">Atendimento mais rápido</div><div class="badge">Menos perguntas repetidas</div><div class="badge">Mais precisão no agendamento</div></div></div>';
        echo '</div>';
        echo '<div class="grid cols-4" style="margin-top:16px"><div class="panel soft"><strong>1</strong><p class="muted" style="margin:6px 0 0">Dados básicos</p></div><div class="panel soft"><strong>2</strong><p class="muted" style="margin:6px 0 0">Endereço</p></div><div class="panel soft"><strong>3</strong><p class="muted" style="margin:6px 0 0">Anamnese</p></div><div class="panel soft"><strong>4</strong><p class="muted" style="margin:6px 0 0">Consentimentos</p></div></div>';
