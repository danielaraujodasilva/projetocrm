...
if ($page === 'studio_settings') {
    ...
    render_studio_shell('Configurações do estúdio', 'Regras comerciais e preparação dos módulos de IA/WhatsApp.', 'settings', function () use ($studio) {
        ...
        echo '<script>(function(){const form=document.getElementById("studioSettingsForm");const body=document.getElementById("settingsOverlayBody");function wrapOverlayForm(){if(!body||!form||body.querySelector("#settingsOverlayForm"))return;const panel=body.querySelector(".settings-panel");const panelKey=(panel&&panel.dataset&&panel.dataset.settingsPanel)||"";if(!panelKey)return;const wrapper=document.createElement("form");wrapper.id="settingsOverlayForm";wrapper.method="post";wrapper.action=form.getAttribute("action")||location.href;wrapper.innerHTML="<input type=\"hidden\" name=\"action\" value=\"save_studio_settings\"><input type=\"hidden\" name=\"settings_tab\" value=\""+panelKey+"\">"+body.innerHTML;body.innerHTML="";body.appendChild(wrapper);wrapper.querySelectorAll("[data-settings-submit]").forEach((btn)=>{btn.type="submit";btn.removeAttribute("data-settings-submit");});}document.addEventListener("click",(event)=>{const openButton=event.target instanceof Element ? event.target.closest("[data-settings-overlay]") : null;if(!openButton)return;setTimeout(wrapOverlayForm,0);}, true);})();</script>';
