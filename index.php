...
if ($action === 'studio_login') {
        ...
        if ($action === 'update_whatsapp_learning_import') {
            $studio = require_studio();
            if (!studio_current_user_is_admin()) {
                throw new RuntimeException('Apenas administradores podem editar aprendizados importados.');
            }
            csrf_verify();
            header('Content-Type: application/json; charset=utf-8');
            $importId = max(0, (int)($_POST['import_id'] ?? 0));
            $title = trim((string)($_POST['learning_title'] ?? ''));
            $learnedText = trim((string)($_POST['learned_text'] ?? ''));
            if (!studio_whatsapp_learning_update_import($studio, $importId, $title, $learnedText)) {
                http_response_code(404);
                echo json_encode(['ok' => false, 'error' => 'Aprendizado não encontrado ou sem alterações.'], JSON_UNESCAPED_UNICODE);
                exit;
            }
            studio_event((int)$studio['id'], 'ai_learning_import_updated', 'Aprendizado da IA revisado pelo administrador.', [
                'category' => 'settings',
                'target_type' => 'whatsapp_ai_learning_import',
                'target_id' => $importId,
            ]);
        ...
        if ($action === 'create_whatsapp_learning_manual') {
            $studio = require_studio();
            if (!studio_current_user_is_admin()) {
                throw new RuntimeException('Apenas administradores podem acrescentar aprendizados.');
            }
            csrf_verify();
            header('Content-Type: application/json; charset=utf-8');
            $user = studio_current_user();
            $import = studio_whatsapp_learning_create_manual(
                $studio,
                trim((string)($_POST['learning_title'] ?? '')),
                trim((string)($_POST['learned_text'] ?? '')),
                (int)($user['id'] ?? 0)
            );
            studio_event((int)$studio['id'], 'ai_learning_manual_added', 'Novo aprendizado manual adicionado à IA.', [
                'category' => 'settings',
                'target_type' => 'whatsapp_ai_learning_import',
                'target_id' => (int)($import['id'] ?? 0),
            ]);
            echo json_encode([
                'ok' => true,
                'import' => $import,
    ...
    render_studio_shell('Roteiro do atendimento', 'Um caminho fixo para coletar os dados e concluir o agendamento sem a IA improvisar etapas.', 'settings', function () use ($studio, $flowConfig, $flowSteps) {
        $clientSteps = array_map(static function (array $step): array {
            return [
                ...
                'ai_rephrase_enabled' => !empty($step['ai_rephrase_enabled']),
        ...
        echo '<div class="flow-ai-rephrase-card"><label class="flow-ai-rephrase-toggle"><input type="checkbox" data-flow-check="ai_rephrase_enabled"><span class="flow-ai-rephrase-switch" aria-hidden="true"></span><span class="flow-ai-rephrase-copy"><strong>Permitir reformulação natural pela IA</strong><small>Quando ligado, a IA pode explicar esta mensagem com suas próprias palavras, mantendo exatamente datas, horários, valores, Pix, nomes, variáveis, opções e instruções. Desligado, o bloco é enviado exatamente como escrito.</small></span></label></div>';
...
if ($page === 'studio_settings') {
    ...
    render_studio_shell('Configurações do estúdio', 'Regras comerciais e preparação dos módulos de IA/WhatsApp.', 'settings', function () use ($studio) {
        ...
        echo '<div class="panel soft" style="margin-top:14px" data-learning-library><div class="actions" style="justify-content:space-between;align-items:flex-start;gap:12px"><div><h4 style="margin:0">Histórico de aprendizados</h4><p class="muted" style="margin:5px 0 0">O ZIP original e as mídias são apagados ao fim do processamento. Aqui fica somente o aprendizado textual retido, que você pode revisar, editar, excluir ou complementar.</p></div><span class="badge" data-learning-count>' . h((string)count($learningImports)) . '</span></div>';
        echo '<div class="panel" data-learning-manual style="margin-top:12px;padding:14px;border-color:#cfe5dc;background:#fbfffd"><div class="actions" style="justify-content:space-between;align-items:flex-start;gap:12px"><div><strong>Acrescentar aprendizado manual</strong><p class="muted" style="margin:5px 0 0">Use para registrar uma regra ou uma forma de conduzir o atendimento que você quer que a IA retenha. Isso não altera preços, agenda ou outros dados oficiais.</p></div><span class="pill success"><i class="fa-solid fa-pen-to-square"></i> Editável</span></div><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Título</label><input type="text" data-learning-manual-title maxlength="245" placeholder="Ex.: Como lidar com cliente indeciso"></div><div class="field"><label>Aprendizado</label><textarea rows="4" data-learning-manual-text maxlength="18000" placeholder="Descreva a estratégia, contexto ou resposta que a IA deve aprender..."></textarea></div></div><div class="actions" style="gap:10px;align-items:center;margin-top:10px"><button type="button" class="btn secondary" data-learning-manual-add><i class="fa-solid fa-plus"></i> Adicionar ao histórico</button><span class="muted" data-learning-manual-status>Pronto para adicionar.</span></div></div><div data-learning-list style="display:grid;gap:10px;margin-top:12px">';
        if (!$learningImports) {
        ...
        foreach ($learningImports as $learningImport) {
            ...
            echo '<details style="margin-top:10px"><summary style="cursor:pointer;font-weight:750">Revisar e editar o que a IA aprendeu</summary><div class="grid cols-2" style="margin-top:10px"><div class="field"><label>Título do aprendizado</label><input type="text" data-learning-title maxlength="255" value="' . h((string)($learningImport['original_file_name'] ?? 'Aprendizado importado')) . '"></div><div class="field"><label>Resumo técnico</label><input type="text" data-learning-summary readonly value="' . h((string)($learningImport['summary'] ?? '')) . '"><small class="muted">O resumo técnico é mantido pelo processamento; edite o conteúdo abaixo.</small></div></div><div class="field" style="margin-top:10px"><label>Conteúdo retido pela IA</label><textarea rows="12" maxlength="18000" data-learning-text-input>' . h((string)($learningImport['learned_text'] ?? '')) . '</textarea></div><div class="actions" style="gap:10px;align-items:center;margin-top:10px"><button type="button" class="btn secondary" data-learning-save="' . h((string)$importId) . '"><i class="fa-solid fa-floppy-disk"></i> Salvar alterações</button><span class="muted" data-learning-edit-status>Você pode corrigir ou complementar este texto.</span></div></details></article>';
        }
        echo '</div></div>';
...
<script>
(function(){
  const cardSelector = '[data-learning-card]';
  const listSelector = '[data-learning-list]';
  function scopeOf(el){return (el&&el.closest&&el.closest('.settings-panel'))||document;}
  function csrf(scope){return (scope.querySelector('input[name=csrf_token]')||document.querySelector('input[name=csrf_token]'))?.value||'';}
  async function post(scope, fields){
    const body=new FormData();
    Object.entries(fields).forEach(([key,value])=>body.append(key,String(value)));
    body.append('csrf_token',csrf(scope));
    const response=await fetch(window.location.pathname+window.location.search,{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'},body});
    const data=await response.json().catch(()=>null);
    if(!response.ok||!data||!data.ok)throw new Error((data&&data.error)||'O servidor não concluiu a solicitação.');
    return data;
  }
  function makeField(labelText, type, value, attribute, rows){
    const field=document.createElement('div'); field.className='field';
    const label=document.createElement('label'); label.textContent=labelText; field.appendChild(label);
    const input=document.createElement(type==='textarea'?'textarea':'input');
    if(type!=='textarea')input.type='text'; if(rows)input.rows=rows; input.maxLength=18000; input.value=value||''; input.dataset[attribute]=''; field.appendChild(input);
    return field;
  }
  function addEditor(card, titleValue, textValue, summaryValue){
    const details=card.querySelector('details'); if(!details||card.querySelector('[data-learning-text-input]'))return;
    details.innerHTML=''; details.style.marginTop='10px';
    const summary=document.createElement('summary'); summary.textContent='Revisar e editar o que a IA aprendeu'; summary.style.cssText='cursor:pointer;font-weight:750'; details.appendChild(summary);
    const grid=document.createElement('div'); grid.className='grid cols-2'; grid.style.marginTop='10px';
    grid.appendChild(makeField('Título do aprendizado','input',titleValue,'learningTitle'));
    const summaryField=makeField('Resumo técnico','input',summaryValue,'learningSummary'); const summaryInput=summaryField.querySelector('input'); summaryInput.readOnly=true; summaryField.querySelector('label').insertAdjacentText('afterend',' '); const hint=document.createElement('small'); hint.className='muted'; hint.textContent='Resumo mantido pelo processamento.'; summaryField.appendChild(hint); grid.appendChild(summaryField); details.appendChild(grid);
    const contentField=makeField('Conteúdo retido pela IA','textarea',textValue,'learningTextInput',12); contentField.style.marginTop='10px'; details.appendChild(contentField);
    const actions=document.createElement('div'); actions.className='actions'; actions.style.cssText='gap:10px;align-items:center;margin-top:10px';
    const save=document.createElement('button'); save.type='button'; save.className='btn secondary'; save.dataset.learningSave=card.dataset.learningCard; save.innerHTML='<i class="fa-solid fa-floppy-disk"></i> Salvar alterações'; actions.appendChild(save);
    const status=document.createElement('span'); status.className='muted'; status.dataset.learningEditStatus=''; status.textContent='Você pode corrigir ou complementar este texto.'; actions.appendChild(status); details.appendChild(actions);
  }
  function decorate(card){
    if(!(card instanceof Element))return;
    const legacy=card.querySelector('[data-learning-text]');
    if(!card.querySelector('[data-learning-text-input]')&&legacy){
      const title=card.querySelector('strong')?.textContent||'Aprendizado importado';
      addEditor(card,title,legacy.textContent||'', '');
    }
  }
  ...
  function createCard(item){
    ...
    const title=document.createElement('div'); title.style.minWidth='0'; const strong=document.createElement('strong'); strong.style.cssText='display:block;overflow-wrap:anywhere'; strong.textContent=item.original_file_name||'Aprendizado manual'; const small=document.createElement('small'); small.className='muted'; small.textContent='Adicionado agora'; title.append(strong,small);
    const remove=document.createElement('button'); remove.type='button'; remove.className='btn tiny danger'; remove.dataset.learningDelete=String(item.id||item.import_id||''); remove.innerHTML='<i class="fa-solid fa-trash"></i> Excluir'; head.append(title,remove); card.appendChild(head);
    const badges=document.createElement('div'); badges.className='actions'; badges.style.cssText='gap:6px;margin-top:10px'; const badge=document.createElement('span'); badge.className='badge'; badge.textContent='aprendizado manual'; badges.appendChild(badge); card.appendChild(badges);
    const details=document.createElement('details'); card.appendChild(details); addEditor(card,item.original_file_name||'Aprendizado manual',item.learned_text||'',item.summary||''); return card;
  }
  const observer=new MutationObserver((mutations)=>{mutations.forEach((mutation)=>mutation.addedNodes.forEach((node)=>{if(node instanceof Element){if(node.matches(cardSelector))decorate(node);node.querySelectorAll?.(cardSelector).forEach(decorate);}}));updateCount();});
  observer.observe(document.body,{childList:true,subtree:true});
  document.addEventListener('click',async(event)=>{
    const save=event.target instanceof Element?event.target.closest('[data-learning-save]'):null;
    if(save){
      const card=save.closest(cardSelector); const scope=scopeOf(save); const status=card?.querySelector('[data-learning-edit-status]'); if(!card)return;
      save.disabled=true; if(status)status.textContent='Salvando...';
      try{await post(scope,{action:'update_whatsapp_learning_import',import_id:save.dataset.learningSave||card.dataset.learningCard,learning_title:card.querySelector('[data-learning-title]')?.value||'',learned_text:card.querySelector('[data-learning-text-input]')?.value||''});if(status)status.textContent='Alterações salvas e já aplicadas às próximas respostas.';}
      catch(error){if(status)status.textContent=error.message||'Não foi possível salvar.';alert(error.message||'Não foi possível salvar.');}
      finally{save.disabled=false;} return;
    }
    const add=event.target instanceof Element?event.target.closest('[data-learning-manual-add]'):null;
    if(add){
      const panel=add.closest('[data-learning-manual]'); const scope=scopeOf(add); const title=panel?.querySelector('[data-learning-manual-title]')?.value||''; const text=panel?.querySelector('[data-learning-manual-text]')?.value||''; const status=panel?.querySelector('[data-learning-manual-status]');
      add.disabled=true; if(status)status.textContent='Salvando aprendizado...';
      try{const data=await post(scope,{action:'create_whatsapp_learning_manual',learning_title:title,learned_text:text});const list=scope.querySelector(listSelector)||document.querySelector(listSelector);list?.querySelector('[data-learning-empty]')?.remove();if(list){list.prepend(createCard(data.import||{}));}if(panel?.querySelector('[data-learning-manual-title]'))panel.querySelector('[data-learning-manual-title]').value='';if(panel?.querySelector('[data-learning-manual-text]'))panel.querySelector('[data-learning-manual-text]').value='';if(status)status.textContent='Aprendizado adicionado ao histórico e aplicado à IA.';updateCount();}
      catch(error){if(status)status.textContent=error.message||'Não foi possível adicionar.';alert(error.message||'Não foi possível adicionar.');}
      finally{add.disabled=false;} return;
    }
  });
  decorateAll(); updateCount();
})();
</script>
HTML;
        echo '</div>';
...
function appointment_calendar_detail_payload(array $appointment): array
{
    ...
    return [
        ...
        'conversation_id' => (int)($appointment['whatsapp_conversation_id'] ?? 0),
        'conversation_url' => (int)($appointment['whatsapp_conversation_id'] ?? 0) > 0
            ? app_url('studio_whatsapp_mobile', ['id' => (int)$appointment['whatsapp_conversation_id']])
            : '',
